#!/usr/bin/env node
// Bank generator for Diag, the daily diagonal sudoku (Sudoku X).
//
//   node scripts/gen-diag.mjs --until 2026-11-30 [--from 2026-09-08] [--seed 20260908] [--fresh]
//
// It APPENDS to app/diag/puzzles.js and never rewrites a board that is
...
// that already reaches that date is a no-op rather than a second helping. With
// no bank on disk it authors one from --from (the launch build only).
//
...
// and not shared with scripts/verify-diag.mjs, to
//   * have EXACTLY ONE solution under the diagonal rule, counted by an
//     exhaustive solver,
//   * fall to LOGIC WITH NO GUESSING at exactly the level its weekday pins, and
//   * satisfy DIAGONAL NECESSITY: with the two diagonals switched off the same
//     clues admit more than one grid, so the rule is never decoration.
// verify-diag.mjs re-proves all of it with its own independent solvers and
// trusts none of the stored fields. Keep the two implementations apart: the
// only thing they may share is the rulebook below.
//
// A DIAGONAL IS A HOUSE, and that is the whole difference from Knight. Each of
// the two long diagonals holds every digit exactly once, so it carries the full
// "these nine cells hold 1 to 9" guarantee: hidden singles, locked candidates
// and subsets all read off a diagonal exactly as off a row. The diagonals are
// in UNITS (29 houses, not 27) as well as in SEE. Unlike the knight set, this
// rule ADDS deductions rather than only eliminations, which is why the board
// can print fewer clues than a plain sudoku and still fall to singles.
//
// THE WEEKDAY RAMP, both knobs PINNED rather than capped:
//
//   day  clues  level
//   Mon   26      1     naked and hidden singles, and nothing harder
//   Tue   24      1
//   Wed   22      1
//   Thu   20      1
//   Fri   18      2     also locked candidates and naked/hidden subsets
//   Sat   16      2
//   Sun   14      2     <- Sunday Edition, the fewest clues of the week
//
// Level is pinned because a ceiling is not a pin: dig an 18-clue board at
// random and a fair share of them still fall to singles alone, which is a
...
//
// HOW A BOARD IS BUILT. Fill a random diagonal grid (MRV backtracking over the
// 29 houses), then dig clues out of it in a random order, keeping a removal
// only while the board still falls to logic AT THE DAY'S LEVEL. That invariant
// is what makes the ramp exact: a Monday dug under the singles-only solver is
// singles-only by construction at whatever depth it stops, and a Friday dug
// under the full toolkit is then re-tested against the singles-only solver and
// thrown away if it never needed the harder move. Digging stops at the day's
// clue count exactly.
//
// A GREEDY DIG DOES NOT REACH THE WEEKEND. Measured on 180 random digs at
// level 2, the greedy floor is 17 to 23 clues (median 19): a 16 comes once in
// ~165 attempts and a 14 never. The diagonals only touch 17 of the 81 cells,
// so the rule bites far less than the knight rule, whose greedy floor was 12
// to 19. So a dig that stalls above the day's count is not thrown away but
// PERTURBED: put two to five random clues back, re-dig in a fresh order, keep
// the result when it is no worse, repeat (iterated local search). That takes
// a 16 in a second or two and a 14 in ten to twenty seconds. 13 was not
// reached at level 2 in 2,500 iterations on six grids, which is why the
// Sunday Edition prints FOURTEEN: the fewest clues this rulebook can prove
// without a guess, not a round number chosen to look hard.
//
...
// same shape every day, which is how Rung, Listed and Crux all degraded. The
// ceilings are Knight's, which its shipped bank cleared with room to spare:
//
//   V1  the 81-cell clue MASK never repeats
//   V2  the solution grid never repeats
//   V3  the per-row clue-count signature never repeats
//   V4  the per-box clue-count signature never repeats
//   V5  the clue digit histogram never repeats
//   V6  no two boards share more than 16 clue POSITIONS
//   V7  no two solutions agree in more than 28 of 81 cells
//   V8  no two solutions are the same grid up to relabelling and
//       the eight symmetries of the square, which is the group the
//       diagonal pair actually survives (band swaps do not)
//   V9  at most 2 boards in the bank carry a clue pattern with
//       180-degree or mirror symmetry
//   V10 per board: no digit is printed more than 6 times, and no
//       row or column carries more than 6 clues
//
...
const BANK = process.env.DIAG_BANK || path.join(process.cwd(), 'app/diag/puzzles.js');
const UNTIL = arg('--until', '2026-11-30');
const SEED = Number(arg('--seed', 20260908));
const FROM = arg('--from', null);
const CACHE_DIR = '/tmp/build';
const CACHE = path.join(CACHE_DIR, 'diag-bank.jsonl');
...
// The two long diagonals: r === c, and r + c === 8. Both are HOUSES.
const onMain = (i) => rowOf(i) === colOf(i);
const onAnti = (i) => rowOf(i) + colOf(i) === 8;
// HOUSE: the cells sharing a row, column or box — an ordinary sudoku's peers.
// Kept separately so diagonal necessity can be measured with the rule off.
const HOUSE = Array.from({ length: 81 }, (_, i) => {
  const out = [];
  for (let j = 0; j < 81; j++) if (j !== i && (rowOf(j) === rowOf(i) || colOf(j) === colOf(i) || boxOf(j) === boxOf(i))) out.push(j);
  return out;
});
// DIAG: the cells sharing a diagonal with i (none for an off-diagonal cell,
// eight for most diagonal cells, sixteen for the centre).
const DIAG = Array.from({ length: 81 }, (_, i) => {
  const out = [];
  for (let j = 0; j < 81; j++) if (j !== i && ((onMain(i) && onMain(j)) || (onAnti(i) && onAnti(j)))) out.push(j);
  return out;
});
const SEE = Array.from({ length: 81 }, (_, i) => [...new Set(HOUSE[i].concat(DIAG[i]))]);
// 29 houses: 9 rows, 9 columns, 9 boxes, and the two diagonals.
const UNITS = (() => {
  const u = [];
  ...
  u.push(Array.from({ length: 9 }, (_, k) => k * 9 + k));
  u.push(Array.from({ length: 9 }, (_, k) => k * 9 + (8 - k)));
  return u;
})();
const UNITS_OF = (() => {
...
// ---- a random full diagonal grid -----------------------------------------
// MRV backtracking with forward checking over SEE, so the diagonals are
// enforced while the grid is built rather than filtered for afterwards. The
// search carries a node cap and simply restarts on a dead end.
...
// Branches on the cell with the fewest candidates. `diag` off drops the two
// diagonals and leaves an ordinary sudoku, which is how diagonal necessity is
// measured: the same clues must then admit at least two grids.
function countSolutions(given, cap, diag) {
  const see = diag ? SEE : HOUSE;
...
// Placements eliminate over SEE and every PATTERN below reads UNITS, which
// here INCLUDES the two diagonals: a diagonal is a house.
...
// solvable at `level` after every removal: a greedy pass first, then the
// iterated local search described in the header. Returns null when the
// iteration budget runs out above `printed`.
const ILS_ITERATIONS = 3000;
function digBoard(sol, printed, level, rnd) {
  const key = sol.join(',');
  const ok = (given) => { const r = logicSolve(given, level); return r.solved && r.grid.join(',') === key; };
  const greedy = (given) => {
    let count = given.filter(Boolean).length;
    for (const i of shuffle([...Array(81).keys()], rnd)) {
      if (count <= printed) break;
      if (!given[i]) continue;
      const v = given[i];
      given[i] = 0;
      if (ok(given)) count--; else given[i] = v;
    }
    return count;
  };
  let best = sol.slice();
  let bc = greedy(best);
  for (let it = 0; it < ILS_ITERATIONS && bc > printed; it++) {
    const cand = best.slice();
    const empties = shuffle([...Array(81).keys()].filter((i) => !cand[i]), rnd);
    const back = 2 + Math.floor(rnd() * 4);
    for (let a = 0; a < back && a < empties.length; a++) cand[empties[a]] = sol[empties[a]];
    const c = greedy(cand);
    if (c <= bc) { best = cand; bc = c; }
  }
  return bc === printed ? best : null;
}
...
// Canonical form of a solution under the group the diagonal pair survives:
// the eight symmetries of the square, times a relabelling of the digits.
// Band and stack swaps are NOT in it — they preserve sudoku but break the X.
...
if (fs.existsSync(BANK)) {
  ...
  if (!text.endsWith('];\n')) { console.error('gen-diag: the bank does not end in "];\\n"; refusing to splice.'); process.exit(1); }
  ({ PUZZLES } = await import(pathToFileURL(BANK).href + `?t=${Date.now()}`));
} else if (!FROM) {
  console.error(`gen-diag: ${BANK} is missing and no --from was given. Appending to a live bank keeps the frozen boards' exact bytes; authoring one from nothing needs an explicit launch date.`);
  process.exit(1);
}
...
if (days <= 0) {
  console.error(`gen-diag: the bank already reaches ${UNTIL}. Nothing to do.`);
  process.exit(0);
}
...
for (let k = 0; k < days; k++) {
  ...
  for (let att = 1; att <= MAX_ATTEMPTS; att++) {
    ...
    if (countSolutions(dug, 2, true) !== 1) { note('not unique under the diagonal rule'); continue; }
    if (countSolutions(dug, 2, false) < 2) { note('diagonal rule was decoration'); continue; }
  ...
  if (!given) {
    ...
    console.error(`gen-diag: no board for #${num} ${live} (${printed} clues, level ${level}) in ${MAX_ATTEMPTS} attempts.`);
...
const HEADER = `// Puzzle data for Diag, the daily diagonal sudoku (Sudoku X). Imported ONLY by
...
fs.writeFileSync(BANK, (text ? text.slice(0, text.length - 3) : HEADER) + body + '\n];\n');
