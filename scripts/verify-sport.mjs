// Structural gate for the Sport bank. Run after every authoring pass:
//
//   node scripts/verify-sport.mjs
//
...
// A day is five tiers of five, and every tier block runs the same five sports
...
// it (the 1998 World Cup final asked from both ends, both answering Brazil). A
// warning rather than a failure: in sport a handful of clubs and countries are
// the fair answer to dozens of genuinely different questions, so a human
// decides. Expect a long list on this bank and read all of it.
const TEMPLATE = new Set(('which team teams club clubs player players country countrys city cities league season game games match matches final finals championship title titles trophy award awards year years first last most fewest name named known called nickname nicknamed plays play played playing home away stadium arena field court pitch coach manager won win wins winner beat scored score points record records career division conference professional national international what where when many number position sport sports these that this from with only there their they side nation').split(' '));
...
for (const p of PUZZLES) {
  ...
  dates.push(p.live);
  if (p.quizId !== `sport-${Number(p.live.slice(5, 7))}-${Number(p.live.slice(8, 10))}-${p.live.slice(2, 4)}`) fail(`${tag}: quizId ${p.quizId} does not match live date ${p.live}`);
  if (p.sunday) fail(`${tag}: Sport runs no Sunday Edition, so no day may be flagged sunday`);
