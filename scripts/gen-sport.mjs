// scripts/gen-sport.mjs - appends authored days to the Sport bank (app/sport/
// questions.js and app/sport/puzzles.js) from the five per-lane files in
// scripts/sport-lanes/.
//
//   node scripts/gen-sport.mjs [laneDir] [--through YYYY-MM-DD] [--check]
//
// WHY A GENERATOR AND NOT A HAND ASSEMBLY. Four things about a Sport day are
...
// sports lanes in the same order), the ids, the identity fields, and the
...
// unchanged lanes is byte-identical. Same shape as scripts/gen-atlas.mjs and
// scripts/gen-biz.mjs, which did this job for those two banks; the differences
// are the lane list, the quizId prefix and the file paths.
//
...
//
// LANE FILES. scripts/sport-lanes/sport-<slug>.mjs, one per lane, each
...
// bank is lane-major within each tier block: slot 1 is tier 1 NFL, slot 2
// tier 1 NBA, ... slot 25 tier 5 Everything Else, which is the order
// scripts/verify-sport.mjs enforces.
//
...
//
// NO SUNDAY EDITION. Sport runs none, so new days carry no `sunday` field at
...
//
// AFTER RUNNING: node scripts/verify-sport.mjs
import fs from 'fs';
...
const Q_FILE = path.join(ROOT, 'app/sport/questions.js');
const P_FILE = path.join(ROOT, 'app/sport/puzzles.js');
...
const LANE_DIR = path.resolve(laneArg || path.join(HERE, 'sport-lanes'));

// The shape of the game: five lanes, always in this order inside every tier
// block. Same list, same order, as scripts/verify-sport.mjs.
const SUBJECTS = ['NFL', 'NBA', 'MLB', 'Soccer', 'Everything Else'];
const LANE_FILE = {
  'NFL': 'sport-nfl.mjs',
  'NBA': 'sport-nba.mjs',
  'MLB': 'sport-mlb.mjs',
  'Soccer': 'sport-soccer.mjs',
  'Everything Else': 'sport-everything-else.mjs',
};
const PER_TIER = SUBJECTS.length;   // 5
const TOTAL_Q = PER_TIER * 5;       // 25
const MONTHS = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

const die = (m) => { console.error(`gen-sport: ${m}`); process.exit(1); };
...
function columnPlan(dayNum, offset) {
  const rnd = rng(seedOf(`sport:cols:${offset}:${dayNum}`));
...
// verifier screens them from SPORT_COPY_FROM onward and a lane that imports one
...
for (let d = 0; d < nDays; d++) {
  ...
  for (let t = 1; t <= 5; t++) for (let s = 0; s < PER_TIER; s++) {
    ...
    const rnd = rng(seedOf(`sport:choices:${startDay}:${id}`));
  ...
  pEntries.push([
    '  {',
    `    num: ${num},`,
    `    quizId: 'sport-${Number(m)}-${Number(dd)}-${y.slice(2)}',`,
    `    live: '${live}',`,
...
if (CHECK) {
  ...
  if (qSame && pSame) { console.log(`gen-sport --check: days ${startDay}..${startDay + nDays - 1} in the bank are byte-identical to what the lanes produce`); process.exit(0); }
  console.error(`gen-sport --check: the bank DIFFERS from the lanes (questions.js ${qSame ? 'ok' : 'differs'}, puzzles.js ${pSame ? 'ok' : 'differs'})`);
  process.exit(1);
}

fs.writeFileSync(Q_FILE, qOut);
fs.writeFileSync(P_FILE, pOut);
console.log(`gen-sport: +${qLines.length} questions, +${pEntries.length} days (${startDay}..${startDay + nDays - 1}), ${dates[0]} to ${dates[dates.length - 1]}`);
