// scripts/gen-atlas.mjs - appends authored days to the Atlas bank (app/atlas/
// questions.js and app/atlas/puzzles.js) from the five per-subject lane files
// in scripts/atlas-lanes/.
//
//   node scripts/gen-atlas.mjs [laneDir] [--through YYYY-MM-DD] [--check]
//
// WHY A GENERATOR AND NOT A HAND ASSEMBLY. Four things about an Atlas day are
...
// subjects in the same order), the ids, the identity fields, and the per-day
// correct-answer column spread. Lane authors therefore write only the question,
// its true answer and three distractors, always in that order, and never choose
// a column; this script owns everything else. Re-running it on unchanged lanes
// is byte-identical.
//
...
//
// LANE FILES. scripts/atlas-lanes/atlas-<slug>.mjs, one per subject, each
// exporting LANE_DAYS: an array of N days, each day five questions in tier
// order 1..5, shaped
//
//   { c: '<subject>', t: 1..5, q: '...', a: '<true answer>', d: ['<wrong>' x3] }
//
// All five lanes must carry the same number of days. Day i of the assembled
// bank is lane-major within each tier block: slot 1 is tier 1 Capitals, slot 2
// tier 1 Physical World, ... slot 25 tier 5 Countries & Peoples, which is the
// order scripts/verify-atlas.mjs enforces.
//
...
//
// NO SUNDAY EDITION. Atlas runs none, so new days carry no `sunday` field at
// all (CLAUDE.md, "Adding a BRAND NEW daily game"); the verifier fails any day
// that sets one. Days 1-41 shipped with an always-false `sunday` and keep it,
...
//
// AFTER RUNNING: node scripts/verify-atlas.mjs
import fs from 'fs';
...
const Q_FILE = path.join(ROOT, 'app/atlas/questions.js');
const P_FILE = path.join(ROOT, 'app/atlas/puzzles.js');
...
const LANE_DIR = path.resolve(laneArg || path.join(HERE, 'atlas-lanes'));

// The shape of the game: five subjects, always in this order inside every tier
// block. Same list, same order, as scripts/verify-atlas.mjs.
const SUBJECTS = ['Capitals', 'Physical World', 'Flags & Borders', 'Places & Landmarks', 'Countries & Peoples'];
const LANE_FILE = {
  'Capitals': 'atlas-capitals.mjs',
  'Physical World': 'atlas-physical-world.mjs',
  'Flags & Borders': 'atlas-flags-borders.mjs',
  'Places & Landmarks': 'atlas-places-landmarks.mjs',
  'Countries & Peoples': 'atlas-countries-peoples.mjs',
};
const PER_TIER = SUBJECTS.length;   // 5
const TOTAL_Q = PER_TIER * 5;       // 25
const MONTHS = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

const die = (m) => { console.error(`gen-atlas: ${m}`); process.exit(1); };
...
function columnPlan(dayNum, offset) {
  const rnd = rng(seedOf(`atlas:cols:${offset}:${dayNum}`));
...
for (let d = 0; d < nDays; d++) {
  ...
  for (let t = 1; t <= 5; t++) for (let s = 0; s < PER_TIER; s++) {
    ...
    if (entry.c !== subject) die(`${id}: lane says "${entry.c}", the subject cycle wants "${subject}"`);
    ...
    const rnd = rng(seedOf(`atlas:choices:${startDay}:${id}`));
  ...
  pEntries.push([
    '  {',
    `    num: ${num},`,
    `    quizId: 'atlas-${Number(m)}-${Number(dd)}-${y.slice(2)}',`,
    `    live: '${live}',`,
...
if (CHECK) {
  ...
  if (qSame && pSame) { console.log(`gen-atlas --check: days ${startDay}..${startDay + nDays - 1} in the bank are byte-identical to what the lanes produce`); process.exit(0); }
  console.error(`gen-atlas --check: the bank DIFFERS from the lanes (questions.js ${qSame ? 'ok' : 'differs'}, puzzles.js ${pSame ? 'ok' : 'differs'})`);
  process.exit(1);
}

fs.writeFileSync(Q_FILE, qOut);
fs.writeFileSync(P_FILE, pOut);
console.log(`gen-atlas: +${qLines.length} questions, +${pEntries.length} days (${startDay}..${startDay + nDays - 1}), ${dates[0]} to ${dates[dates.length - 1]}`);
