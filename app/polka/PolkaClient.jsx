...
// Polka — the daily kropki sudoku.
//
// Not one digit is printed, only dots. A WHITE dot between two squares means
// their digits differ by exactly 1. A BLACK dot means one is double the
// other. NO dot means NEITHER is true, and those empty edges are clues too.
// (A 1 next to a 2 fits both rules, so either dot may be printed there.)
// Rows, columns and boxes still hold 1 to 9 exactly once, and the dot pattern
// pins exactly one solution, always reachable by pure logic. Classic sudoku
// manners: a wrong digit is NOT flagged, it looks like any other entry, and
// the grid is accepted only once every square is right.
//
// A solve is a flat 10 and the daily leaderboard is a race on the clock, the
// same lane as every sudoku on the slate. Forked from app/polka/PolkaClient.jsx
// re-based on 9×9 geometry with the dot overlay (DotsOverlay below) and no
// givens at all. Same daily plumbing as every other board: banked puzzles
// gated by Eastern date on the server (app/polka/page.js), per-puzzle
// localStorage saves, /polka?p=N archive pinning, streaks, and the shared
// /api/quiz/* flow.
...
// Polka identity is KELLY GREEN, clear of every other sudoku (Suds orange,
// Quilt magenta, Cages purple, Sando teal, Polka blue, Mercury red) and a
// full step brighter than Tally's forest. Every colour the board uses derives
// from COLORS.accent below rather than a literal hex a second time (the
...
const COLORS = {
  ...
  accent: ACCENT,
  accentSoft: '#ecf9f1',
  accentTint: '#d3f2e0',       // a square holding the highlighted digit
  accentPick: '#b4e8cb',       // the selected square
...
const STATS_KEY = 'sot_polka_stats';
...
function DotsOverlay({ dots, n }) {
  ...
  for (let r = 0; r < n; r++) {
    for (let c = 0; c < n - 1; c++) {
      const t = dots.h[r][c];
      if (t) out.push({ key: `h${r}-${c}`, x: c + 1, y: r + 0.5, t });
    }
  }
  for (let r = 0; r < n - 1; r++) {
    for (let c = 0; c < n; c++) {
      const t = dots.v[r][c];
      if (t) out.push({ key: `v${r}-${c}`, x: c + 0.5, y: r + 1, t });
    }
  }
  return (
...
export default function PolkaClient({ puzzles = [], forceNum = null }) {
  const PUZZLE = useMemo(() => pickPuzzle(puzzles, forceNum), [puzzles, forceNum]);
  // Polka prints no digits at all; GIVEN stays as an all-empty grid so every
  // shared code path (pads, hints, erase guards) holds unchanged.
  const GIVEN = useMemo(() => Array.from({ length: N }, () => Array(N).fill(0)), []);
  const SOL = PUZZLE.sol;
  const STORE_KEY = `sot_polka_${PUZZLE.num}`;
  ...
  useEffect(() => { if (stats) setHintOk(hintAllowed('polka', stats)); }, [stats]);
  useEffect(() => { if (g.hintUsed) spendHint('polka'); }, [g.hintUsed]);
  ...
  const LOFT = isLoft('polka');
  const STAGE = isStage('polka', searchParams);
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('polka');
  const Cap = STAGE ? StageChrome : LoftCap;
  const STAGE_ACC = { '--stg-acc-dk': gameColor('polka'), '--stg-acc-lt': gameColorLight('polka'), '--stg-onramp-lt': gameOnrampLight('polka'), '--stg-acc-ink-lt': gameAccentInkLight('polka') };
  ...
  const iq = useIqStanding({ game: 'polka', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'polka', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'polka', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'polka', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'polka', active: LOFT && !playing });
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_polka_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_polka_day');
      }
    } catch (e) {}
  ...
  const REC_KEY = `sot_polka_rec_${PUZZLE.num}`;
  ...
  function shareText() {
    // A row of polka dots. No board state leaks: it is all nine or none,
    // exactly like the score.
    const squares = won ? '⚫⚪⚫⚪⚫⚪⚫⚪⚫' : '⬜'.repeat(N);
    const hintBit = g.hintUsed ? ' · \u{1F4A1}' : '';
    const streakBit = isTodays && myStats.cur >= 2 ? ` · streak ${myStats.cur}` : '';
    const head2 = won
      ? `Polka #${PUZZLE.num}${PUZZLE.sunday ? ' · Sunday' : ''} · solved in ${elapsed}${hintBit}${streakBit}`
      : `Polka #${PUZZLE.num} · gave up`;
    return `${head2}\n${squares}\n${shareUrl()}`;
  }
  function shareUrl() {
    return withRef(`mindloftdaily.com/polka${isTodays ? '' : `?p=${PUZZLE.num}`}`);
  }
  function copyShare() {
    const text = playing
      ? `Polka #${PUZZLE.num} — the daily kropki sudoku from Mind Loft.\n${shareUrl()}`
      : shareText();
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="No digits are printed, only dots. A white dot between two squares means their digits differ by exactly 1. A black dot means one is double the other. No dot means NEITHER is true, and those empty edges are clues too. Rows, columns and boxes still hold 1 to 9 exactly once."
      steps={[
        ...
        <><b>Undo</b> (or Ctrl+Z) takes back your last move. <b>Clear</b> wipes every number you have entered.</>,
      ]}
      knack="A 1 next to a 2 fits both rules, so either dot may be printed there, and the one trap is assuming which. The silences do half the solving: a square whose edges carry no dots can sit next to nothing consecutive and nothing double."
      footer="Every deal has exactly one solution and can always be reached by logic, with no guessing. Solve the grid and you score a perfect 10, and because nothing is counted against you, the daily leaderboard is a straight race on the clock. One free hint, on your first ever play, fills a correct number. Sundays are the hardest deal of the week."
    />
  );
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && <Grain />}
      {!STAGE && (
      <DailyChrome slug="polka" name="Polka" collapsed={started} loft={LOFT} />
      )}
      {/* LOFT: the cap replaces the title block AND the board's own stat strip.
          Polka is scored all or nothing, so the outcome is only ever won or
      ...
      {LOFT && (
        <Cap gameKey="polka" quizId={PUZZLE.quizId}
          ...
          sunday={PUZZLE.sunday ? 'Sunday Edition · Hardest deal' : null}
          figures={[
            { v: elapsed, k: 'time' },
      ...
      <div className="pk-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style dangerouslySetInnerHTML={{ __html: `
          @media(max-width:560px){.pk-wrap{padding-left:12px !important;padding-right:12px !important;}}
          ...
          .pk-btn:hover{background:var(--stg-surf2, ${COLORS.accentSoft});}
          .pk-cell{display:flex;align-items:center;justify-content:center;font-family:${MONO};box-sizing:border-box;cursor:pointer;position:relative;user-select:none;-webkit-tap-highlight-color:transparent;min-width:0;min-height:0;overflow:hidden;}
          .pk-given{font-weight:700;color:${INK};}
          .pk-user{font-weight:500;color:var(--stg-acc-ink, ${COLORS.accent});}
          .pk-notes{display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);width:100%;height:100%;padding:2px;box-sizing:border-box;}
          .pk-note{display:flex;align-items:center;justify-content:center;font-family:${MONO};font-size:9px;line-height:1;color:var(--stg-ink2, #8a93a3);}
          .pk-pad{width:100%;aspect-ratio:1;border-radius:9px;border: 1.5px solid var(--stg-line, rgba(28,30,36,0.5));background:${STAGE ? 'var(--stg-surf)' : 'var(--white)'};font-family:${MONO};font-weight:500;color:${INK};cursor:pointer;display:flex;align-items:center;justify-content:center;position:relative;box-shadow:0 2px 0 rgba(28,30,36,0.4);}
          .pk-pad:active{transform:translateY(1px);box-shadow:0 1px 0 rgba(28,30,36,0.4);}
          .pk-pad.done{color:var(--stg-mute2, #c3c8cf);box-shadow:none;background:${STAGE ? 'var(--stg-surf2)' : '#f4f5f7'};cursor:default;}
          .pk-pad.done span{text-decoration:line-through;}
          .pk-pad.armed{background:var(--stg-acc, ${COLORS.accent});color:var(--stg-onramp, var(--white));border-color:var(--stg-acc, ${COLORS.accent});box-shadow:0 2px 0 var(--stg-acc, ${COLORS.accentDeep});}
          .pk-pad.armed .pk-pad-n{color:${STAGE ? 'color-mix(in srgb, var(--stg-onramp, #08222e) 68%, transparent)' : COLORS.accentTint};}
          .pk-pad .pk-pad-n{position:absolute;bottom:2px;right:4px;font-size:8px;color:var(--stg-mute2, #aab0bb);font-weight:500;}
          ...
          .pk-tool.on{background:${STAGE ? STAGE_C : COLORS.ink};color:${STAGE ? 'var(--stg-onramp, #08222e)' : 'var(--white)'};border-color:${STAGE ? STAGE_C : COLORS.ink};}
        ` }} />
        ...
        <DailyMasthead
          ...
          marginBottom={16}
          onHelp={() => setShowHelp(true)}
          sunday={PUZZLE.sunday && <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: `var(--stg-onramp, ${T.white})`, background: `var(--stg-acc, ${COLORS.accent})`, borderRadius: 4, padding: '2px 6px' }}>Sunday Edition &middot; Hardest deal</span>}
          blocks={'POLKA'.split('').map((ch, i) => (
          ...
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: INK, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>No digits are printed, only dots. A white dot means the two neighbours differ by exactly 1, a black dot means one is double the other, and no dot means neither. Rows, columns and boxes still hold 1 to 9 once each.</p>
              </div>
            )}
          ...
          <div style={{ maxWidth: GRID_MAX, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${N}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${N}, minmax(0, 1fr))`, aspectRatio: '1', border: '2.5px solid var(--stg-line3, rgba(28,30,36,0.85))', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
              {Array.from({ length: CELLS }).map((_, idx) => {
                ...
                const cls = `pk-cell ${given ? 'pk-given' : val ? 'pk-user' : ''}`;
                return (
                  <div key={idx} className={cls} style={cellStyle(idx)}
                    ...
                    {val ? (
                      ...
                      <div className="pk-notes" style={{ position: 'relative', zIndex: 2 }}>
                        {Array.from({ length: N }).map((__, k) => (
                          <span key={k} className="pk-note">{(notes[idx] & (1 << (k + 1))) ? k + 1 : ''}</span>
                        ))}
                      </div>
                    ) : null}
                  </div>
              ...
              <DotsOverlay dots={PUZZLE.dots} n={N} />
            </div>
          </div>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${N}, minmax(0, 1fr))`, gap: 5, maxWidth: GRID_MAX, margin: '16px auto 0' }}>
                {Array.from({ length: N }).map((_, k) => {
                  ...
                  return (
                    <button key={d} className={`pk-pad${done ? ' done' : ''}${armed === d ? ' armed' : ''}`} onClick={() => { if (!done) padTap(d); }} aria-label={`enter ${d}`}>
                      <span style={{ fontSize: 'clamp(15px, 4.5vw, 21px)' }}>{d}</span>
                      {!done && <span className="pk-pad-n">{N - (padCounts[d] || 0)}</span>}
                    </button>
                  );
                })}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center', marginTop: 10, flexWrap: 'wrap' }}>
                <button className={`pk-tool${noteMode ? ' on' : ''}`} onClick={() => setNoteMode((m) => !m)} title="Toggle pencil notes (N)">
                  <Pencil size={14} /> Notes {noteMode ? 'on' : 'off'}
                </button>
                <button className="pk-tool" onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)" style={{ opacity: canUndo ? 1 : 0.4, cursor: canUndo ? 'pointer' : 'default' }}>
                  <RotateCcw size={14} /> Undo
                </button>
                <button className="pk-tool" onClick={() => eraseCell(sel)} title="Erase selected square (Backspace)">
                  <Eraser size={14} /> Erase
                </button>
                <button className="pk-tool" onClick={clearBoard} disabled={!hasEntries}
                ...
                {hintOk && !g.hintUsed && (
                  <button className="pk-tool" onClick={useHint} title="Fill one correct square (one hint, first play only)" style={{ background: `var(--stg-surf, ${COLORS.accentSoft})`, borderColor: 'rgba(22,163,74,0.5)', color: ACC_DEEP_INK }}>
        ...
        {started && (
          ...
          {!playing && (
            <div style={{ maxWidth: GRID_MAX + 76, margin: '0 auto' }}>
              {PUZZLE.sunday && (
                <div style={{ fontSize: 12.5, fontWeight: 600, color: FADED, fontStyle: 'italic', margin: '10px 0 0' }}>The Sunday Edition — a deal from the top of the difficulty distribution.</div>
              )}
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Polka in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new kropki sudoku drops at midnight Eastern.'}
                    {prevPuzzle && (
                      <>
                        {' '}Meanwhile:{' '}
                        <a href={`/polka?p=${prevPuzzle.num}`} style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>
                          play yesterday&rsquo;s Polka &rarr;
                        </a>
                      </>
                    )}
                  </>
                  ...
                  <>
                    ...
                    <a href="/polka" style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Polka &rarr;</a>
                    {' · '}
          ...
          <LoftFinish
            ...
            gameRank={allTime && allTime.ready
              ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '—',
                  label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Polka all time` : 'all-time rank' }
              : null}
            day={dayStats}
            ...
            options={[
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Polka', sub: `No. ${prevPuzzle.num}, yesterday's puzzle`, href: `/polka?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="polka" name="Polka" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          {LOFT && (
            <div className={STAGE ? undefined : 'loft-report'}>
              <ReportIssue self="polka" name="Polka" accent="#ffffff" align="center" onHelp={() => setShowHelp(true)} />
            </div>
          )}
          ...
          <DailyGamesGrid replay={!playing ? resetGame : null}
            ...
            light
            boardSlot={<DailyBoardPanel self="polka" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider
          />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Polka to your Home Screen</div>
              {isIosDevice() ? (
                <ol style={{ margin: '0 0 4px', paddingLeft: 20, color: INK, fontSize: 14, lineHeight: 1.7 }}>
                  <li>Tap the <b>Share</b> button in Safari&apos;s toolbar.</li>
                  <li>Scroll down and tap <b>Add to Home Screen</b>.</li>
                  <li>Tap <b>Add</b> &mdash; the tile opens today&apos;s kropki sudoku, every day.</li>
                </ol>
              ) : (
                <p style={{ margin: '0 0 4px', color: INK, fontSize: 14, lineHeight: 1.7 }}>
                  Open your browser&apos;s menu and choose <b>Add to Home Screen</b> (or <b>Install app</b>). The tile opens today&apos;s kropki sudoku, every day.
                </p>
              )}
