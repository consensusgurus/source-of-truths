...
export default function SurviveStateBoard({ quizId, mobile = false }) {
  ...
  const QSTAGE = isQuizStage('SurviveStateBoard', searchParams);
  ...
  const resultImgUrl = `${SITE_URL}/quiz/${quiz.id}/result-image?s=${score}&t=${total}&p=0`;
    ...
    <div className={QSTAGE ? 'stage-page' : (undefined)}
      ...
      {QSTAGE && (
        <StageChrome
          ...
          dateLabel={`${total} ${total === 1 ? 'prompt' : 'prompts'}`}
      ...
      <div className="qzf-w" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '4px 38px 80px' }}><div className="qzf-line" aria-hidden="true" />
          ...
          <>
            ...
            <div style={{ position: 'sticky', top: 0, zIndex: 24, background: `var(--stg-surf,${COLORS.cream})`, paddingBottom: 4 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, background: `var(--stg-surf2,${COLORS.paper})`, border: `1px solid var(--stg-line,${COLORS.faded}33)`, borderRadius: 12, padding: '16px 20px' }}>
                <div>
                  ...
                  <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: FADED }}>Named</div>
                </div>
            ...
            {phase === 'idle' && (
              <div style={{ textAlign: 'center', padding: '26px 24px 30px', borderRadius: 10, border: `1.5px solid var(--stg-line2,${COLORS.ink})`, background: `var(--stg-surf2,${COLORS.paper})`, marginTop: 8 }}>
                ...
                <p style={{ fontFamily: SANS, fontSize: 15, lineHeight: 1.5, color: 'var(--stg-ink2,#4a4339)', maxWidth: 480, margin: '0 auto 6px' }}>
            ...
            {phase === 'playing' && cur && (
              <div style={{ marginTop: 8 }}>
                <div style={{ textAlign: 'center', padding: '34px 24px 30px', borderRadius: 12, border: `1.5px solid var(--stg-line2,${COLORS.ink})`, background: `var(--stg-surf,${T.white})` }}>
                  <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: `var(--stg-acc-ink,${COLORS.ember})`, marginBottom: 14 }}>{promptLabel} {pos + 1} of {total}</div>
                  <div style={{ fontFamily: SERIF, fontWeight: 800, fontSize: 'clamp(28px, 5vw, 46px)', lineHeight: 1.08, margin: '0 0 6px' }}>{cur.stadium}</div>
                  <div style={{ fontFamily: SANS, fontSize: 14, color: FADED, marginBottom: 22 }}>What {answerNoun} is it in?</div>
                  <input
                    ref={inputRef}
                    value={value}
                    ...
                    style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', textAlign: 'center', fontFamily: SANS, fontSize: 24, fontWeight: 600, padding: '14px 18px', borderRadius: 10, border: `2px solid ${inputBorder}`, background: `var(--stg-surf,${COLORS.cream})`, color: INK, outline: 'none', transition: 'border-color .15s' }}
                  />
                  <div style={{ fontFamily: MONO, fontSize: 11, color: FADED, marginTop: 12 }}>Auto-advances on the right answer. One wrong answer ends it.</div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap', marginTop: 20 }}>
                  {order.map((_, i) => {
                    const done = i < pos;
                    const curi = i === pos;
                    return <span key={i} style={{ width: curi ? 22 : 9, height: 9, borderRadius: 5, background: done ? `var(--stg-good,${COLORS.forest})` : (curi ? `var(--stg-acc,${COLORS.ember})` : `var(--stg-line,${COLORS.faded + '44'})`), transition: 'all .2s' }} />;
                  })}
                </div>
        ...
        {tab === 'share' && (
          <div style={{ textAlign: 'center', padding: '12px 0 8px' }}>
            ...
            <p style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 19, color: INK, maxWidth: 480, margin: '0 auto 20px' }}>{phase === 'done' ? `You named ${score} of ${total}. Challenge someone to beat it.` : `Send this quiz to someone who thinks they know their ${promptLabel.toLowerCase()} geography.`}</p>
