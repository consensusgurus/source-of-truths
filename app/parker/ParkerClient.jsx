...
export default function ParkerClient({ puzzles = [], forceNum = null }) {
  ...
  const STAGE = isStage('park', searchParams);
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('park');
  const Cap = STAGE ? StageChrome : LoftCap;
  const STAGE_ACC = { '--stg-acc-dk': gameColor('park'), '--stg-acc-lt': gameColorLight('park'), '--stg-onramp-lt': gameOnrampLight('park'), '--stg-acc-ink-lt': gameAccentInkLight('park') };
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {LOFT && (
        <Cap gameKey="park" quizId={PUZZLE.quizId}
          name="Parker"
          cat="Logic"
      ...
      <div className="pk-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Parker is ready'}</div>
            {gateRules ? rulesBody : (
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Parker in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new board drops at midnight Eastern.'}
            ...
            <LoftFinish
              ...
              gameRank={allTime && allTime.ready
                ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '\u2014',
                    label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Parker all time` : 'all-time rank' }
                : null}
              day={dayStats}
        ...
        {!STAGE && <GamePanel self="parker" name="Parker" onShow={() => setShowChrome(true)} />}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Parker to your Home Screen</div>
              {isIosDevice() ? (
      ...
      <ValetDoorPop ready={hydrated && preStart && isTodays} self="Parker" />
