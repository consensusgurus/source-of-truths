...
// Atlas — the daily geography gauntlet.
//
// Twenty-five questions about the world stand between you and a perfect run.
// They climb from gimme to expert in five tiers of five, every tier cycling
// the same five subjects (capitals, physical geography, flags and borders,
// places and landmarks, countries and peoples), and everyone faces the same
// twenty-five in the same order. You have twenty seconds a question and one
// life: answer wrong, or let the clock hit zero, and the run is over. Every
// question you clear is a point, so there is never a reason to stop playing
// and never a way back in.
//
// Streak asks whether you know a little about everything and Deep asks how far
// down one subject you can go. Atlas asks how well you know the map. Ties on
...
const COLORS = {
  ...
  accent: '#047857',        // Atlas identity — atlas green
  accentSoft: '#e7f4ee', green: T.successDeep,
};
const SANS = "'Manrope', system-ui, -apple-system, sans-serif";
const MONO = "'DM Mono', ui-monospace, 'SFMono-Regular', monospace";
const HELP_KEY = 'sot_atlas_help_seen';
const STATS_KEY = 'sot_atlas_stats';

const Q_SECONDS = 20;
const TOTAL_Q = 25;
const PER_TIER = 5;
const TIER_NAMES = ['Home ground', 'Farther out', 'Off the main roads', 'The far corners', 'Expert'];
...
export default function AtlasClient({ puzzles = [], questionsByNum = {}, forceNum = null }) {
  ...
  const LOFT = isLoft('atlas');
  const STAGE = isStage('atlas', searchParams);
  ...
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('atlas');
  const STAGE_ACC = { '--stg-acc-dk': gameColor('atlas'), '--stg-acc-lt': gameColorLight('atlas'), '--stg-onramp-lt': gameOnrampLight('atlas'), '--stg-acc-ink-lt': gameAccentInkLight('atlas') };
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_atlas_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_atlas_day');
      }
    } catch (e) {}
  ...
  const iq = useIqStanding({ game: 'atlas', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'atlas', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'atlas', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'atlas', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'atlas', active: LOFT && !playing });
  ...
  const REC_KEY = `sot_atlas_rec_${PUZZLE.num}`;
  ...
  // wipe the saved board and run today's map again as practice. The first
  ...
  function shareUrl() { return withRef(`mindloftdaily.com/atlas${isTodays ? '' : `?p=${PUZZLE.num}`}`); }
  function shareText() {
    ...
    const head = won
      ? `Atlas #${PUZZLE.num} · ran the table, 25/25 · ${elapsed}${streakBit}`
      : `Atlas #${PUZZLE.num} · ${depth} straight · ${elapsed}${streakBit}`;
    return `${head}\n${bar}\n${shareUrl()}`;
  }
  function copyShare() {
    const text = playing
      ? `Atlas #${PUZZLE.num} — the daily geography gauntlet from Mind Loft. Twenty-five questions, one life.\n${shareUrl()}`
      : shareText();
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="Twenty-five questions on the world, one life."
      steps={[
        <>Answer multiple-choice geography until you get one wrong. <b>Every question you clear is a point.</b></>,
        ...
        <>The twenty-five climb in <b>five rounds of five</b>, from gimmes to genuinely obscure, each round cycling the same five subjects: capitals, the physical world, flags and borders, places and landmarks, and countries and peoples.</>,
      ]}
      knack="Every round asks the same five subjects, so you always know what is coming next. The one you are weakest at is the one that ends most of your runs."
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && (
      <DailyChrome slug="atlas" name="Atlas" collapsed={started} loft={LOFT} />
      )}
      ...
      {LOFT && (
        <Cap gameKey="atlas" quizId={PUZZLE.quizId}
          name="Atlas"
          cat="Geography"
      ...
      <div className="at-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Atlas is ready'}</div>
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: INK, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>Twenty-five questions about the world, easy to expert, {Q_SECONDS} seconds each, and one life. Answer until you miss; every question you clear is a point. The clock starts when you do.</p>
              </div>
            )}
            <div style={{ marginTop: 18, display: 'flex', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <button className="at-btn" onClick={startGame} style={{ borderColor: STAGE ? STAGE_C : undefined, background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
              <div>
        ...
        <div className={STAGE ? 'stg-board' : (LOFT ? 'loft-card' : undefined)} style={{ background: STAGE ? SURF : T.white, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 10, padding: '13px 15px 15px', boxShadow: STAGE ? 'none' : '5px 5px 0 rgba(28,30,36,0.16)', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: MONO, fontSize: 11.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: FADED, borderBottom: '1px solid rgba(28,30,36,0.18)', paddingBottom: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <span style={{ whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              <Globe2 size={13} style={{ color: ACC_INK }} />
          ...
          {won && (
            <div style={{ textAlign: 'center', padding: '18px 6px 10px' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: FADED }}>You ran the table in {elapsed}. That is the whole map.</div>
            </div>
          )}
            ...
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: FADED, margin: '6px 0 4px', lineHeight: 1.5 }}>
                {won
                  ...
                  : depth >= 20 ? 'Into the far corners. That is a serious run.'
                  : depth >= 15 ? 'You got off the main roads and kept going.'
                  : depth >= 10 ? 'Through the easy rounds and into real geography.'
                  : depth >= 5 ? 'Home ground is behind you. The map gets unfamiliar fast.'
                  : 'The map claims its share early. Tomorrow is a new run.'}
              </div>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Atlas in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new map drops at midnight Eastern.'}
                    {prevPuzzle && (<>{' '}Meanwhile: <a href={`/atlas?p=${prevPuzzle.num}`} style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>run yesterday&rsquo;s map &rarr;</a></>)}
                  </>
                ) : (
                  <>
            ...
            <LoftFinish
              ...
              gameRank={allTime && allTime.ready
                ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '\u2014',
                    label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Atlas all time` : 'all-time rank' }
                : null}
              day={dayStats}
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Atlas', sub: `No. ${prevPuzzle.num}, yesterday\u2019s puzzle`, href: `/atlas?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="atlas" name="Atlas" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          {LOFT && (
            <div className={STAGE ? undefined : 'loft-report'}>
              <ReportIssue self="atlas" name="Atlas" accent="#ffffff" align="center" onHelp={() => setShowHelp(true)} />
            </div>
          )}
          {!LOFT && (
          <DailyGamesGrid replay={!playing ? resetGame : null} self="atlas" maxWidth={620}
            challengeHref={`/duel/new?quiz=${encodeURIComponent(PUZZLE.quizId)}`}
            share={{ label: copied ? 'Copied' : 'Share', onClick: copyShare }} light
            boardSlot={<DailyBoardPanel self="atlas" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Atlas to your Home Screen</div>
              {isIosDevice() ? (
                <ol style={{ margin: '0 0 4px', paddingLeft: 20, color: INK, fontSize: 14, lineHeight: 1.7 }}>
                  <li>Tap the <b>Share</b> button in Safari&apos;s toolbar.</li>
                  <li>Scroll down and tap <b>Add to Home Screen</b>.</li>
                  <li>Tap <b>Add</b> &mdash; the tile opens today&apos;s map, every day.</li>
                </ol>
              ) : (
                <p style={{ margin: '0 0 4px', color: INK, fontSize: 14, lineHeight: 1.7 }}>Open your browser&apos;s menu and choose <b>Add to Home Screen</b> (or <b>Install app</b>). The tile opens today&apos;s map, every day.</p>
              )}
      ...
      {!playing && !endClosed && !LOFT && (
        <DailyEndCard modal self="atlas" won={won}
          headline={won ? <>You ran the table.</> : depth >= 15 ? <>A serious run.</> : <>The map got you.</>}
          subline={won
      ...
      {showHelp && (
        <div onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: STAGE ? 'var(--stg-raise,#0e131f)' : COLORS.cream, borderRadius: 12, border: STAGE ? '1px solid var(--stg-line)' : `2px solid ${COLORS.ink}`, padding: '20px 22px', fontFamily: SANS, maxHeight: '86vh', overflowY: 'auto' }}>
            ...
            {rulesBody}
            <button className="at-btn" onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }} style={{ marginTop: 14, background: COLORS.ink, color: T.white }}>Play</button>
          </div>
        </div>
      )}
      ...
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: INK }}>About Atlas</h2>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Atlas is a free daily geography game from Mind Loft. Twenty-five multiple-choice questions about the world climb from ones anyone can answer to ones almost nobody can, and a single wrong answer ends the run. Your score is simply how many you cleared in a row, which makes every question a small act of nerve: the further you get, the more you have to lose.
        </p>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Everyone plays the same twenty-five questions in the same order each day, so the daily leaderboard is a straight fight: longest run wins, and ties break by time. Twenty seconds a question keeps it honest. Every round cycles the same five subjects, capitals, the physical world, flags and borders, places and landmarks, and countries and peoples, so a run rewards a whole map rather than one corner of it.
        </p>
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          A new map drops every day at midnight Eastern. No app, no signup, play free in your browser, keep a streak, and race the daily leaderboard. More dailies: <a href="/streak" style={{ color: INK, fontWeight: 800 }}>Streak</a>, our forty-question trivia gauntlet, <a href="/ping" style={{ color: INK, fontWeight: 800 }}>Ping</a>, our daily secret city, and <a href="/span" style={{ color: INK, fontWeight: 800 }}>Span</a>, our daily border chain.
        </p>
      </section>
