...
export default function DeepClient({ puzzles = [], questionsByNum = {}, forceNum = null }) {
  ...
  const STAGE = isStage('deep', searchParams);
  ...
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('deep');
  const STAGE_ACC = { '--stg-acc-dk': gameColor('deep'), '--stg-acc-lt': gameColorLight('deep'), '--stg-onramp-lt': gameOnrampLight('deep'), '--stg-acc-ink-lt': gameAccentInkLight('deep') };
  ...
  const qCard = (qq, dead) => (
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
        <span style={{ fontFamily: MONO, fontSize: 10.5, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500, color: ACC_INK }}>{TOPIC}</span>
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {LOFT && (
        <Cap gameKey="deep" quizId={PUZZLE.quizId}
          name="Deep"
          cat="Trivia"
      ...
      <div className="dp-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .dp-btn{font-family:${SANS};font-weight:800;font-size:14px;border:2px solid ${STAGE ? 'var(--stg-line2)' : 'var(--blue-deep)'};background:${STAGE ? 'transparent' : 'var(--white)'};color:${STAGE ? 'var(--stg-ink)' : 'var(--blue-deep)'};border-radius:8px;padding:9px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:7px;}
          .dp-btn:hover{background:var(--stg-surf2, var(--accent-soft));}
          ...
          .dp-grid:not(.nohov) .dp-choice:not(:disabled):hover{background:var(--stg-surf2, ${COLORS.paper});}
          .dp-timebar{height:7px;border-radius:4px;background:var(--stg-surf, ${COLORS.paper});overflow:hidden;}
          .dp-timefill{height:100%;border-radius:4px;transition:width .1s linear;}
          .dp-topic{font-family:${SANS};font-weight:900;font-size:22px;color:var(--stg-acc-ink, ${COLORS.accent});line-height:1.2;}
        ` }} />
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18, display: 'flex', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <button className="dp-btn" onClick={startGame} style={{ borderColor: STAGE ? STAGE_C : undefined, background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Dive</button>
              <div>
        ...
        <div className={STAGE ? 'stg-board' : (LOFT ? 'loft-card' : undefined)} style={{ background: STAGE ? SURF : T.white, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 10, padding: '13px 15px 15px', boxShadow: STAGE ? 'none' : '5px 5px 0 rgba(28,30,36,0.16)', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: MONO, fontSize: 11.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: FADED, borderBottom: '1px solid rgba(28,30,36,0.18)', paddingBottom: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <span style={{ whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              <Anchor size={13} style={{ color: ACC_INK }} />
          ...
          {won && (
            <div style={{ textAlign: 'center', padding: '18px 6px 10px' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: FADED }}>You hit the bottom of {TOPIC} in {elapsed}. Nobody knows it better today.</div>
            </div>
          )}
        ...
        {started && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(28,30,36,0.10)', flexWrap: 'wrap' }}>
            <span style={{ fontFamily: SANS, fontSize: 12, fontWeight: 700, color: FADED }}>One wrong answer ends the dive. Everything you clear is banked.</span>
          </div>
        )}
        </div>
        )}
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: INK, margin: '8px 0 0' }}>
                {won ? <>A clean dive: <span style={{ color: ACC_INK }}>all 15</span> on {TOPIC}.</> : <>You went <span style={{ color: ACC_INK }}>{depth} of {TOTAL_Q}</span> deep on {TOPIC}.</>}
              </div>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>A new topic in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new topic drops at midnight Eastern.'}
        ...
        {!STAGE && <GamePanel self="deep" name="Deep" onShow={() => setShowChrome(true)} />}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Deep to your Home Screen</div>
              {isIosDevice() ? (
                ...
                <p style={{ margin: '0 0 4px', color: INK, fontSize: 14, lineHeight: 1.7 }}>Open your browser&apos;s menu and choose <b>Add to Home Screen</b> (or <b>Install app</b>). The tile opens today&apos;s topic, every day.</p>
              )}
      ...
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        ...
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          A new topic drops every day at midnight Eastern. No app, no signup, play free in your browser, keep a streak, and race the daily leaderboard. If you would rather range across everything than go down one subject, try <a href="/streak" style={{ color: INK, fontWeight: 800 }}>Streak</a>, our forty-question gauntlet. More dailies: <a href="/redact" style={{ color: INK, fontWeight: 800 }}>Redact</a> and <a href="/listed" style={{ color: INK, fontWeight: 800 }}>Listed</a>.
        </p>
      </section>
