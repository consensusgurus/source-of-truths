...
// Biz, the daily business gauntlet.
//
// Twenty-five business questions stand between you and a perfect run. They climb
// from gimme to expert in five tiers of five, every tier cycling the same five
// lanes (brands, markets, founders, deals and business history), and everyone
...
//
// EVERY FACT IN THE BANK IS FROZEN, and business is the category that most needs
// the rule, because the answer keeps moving: a chief executive resigns, a brand
// changes hands, a "largest" becomes a second largest. So the bank asks only
// about settled things. Never who currently runs or owns anything, never a
// present-tense superlative, never a deal still pending or a case still on
// appeal.
//
// Streak asks whether you know a little about everything and Deep asks how far
// down one subject you can go. Biz asks whether you read the business pages.
...
const COLORS = {
  ...
  accent: '#0f5132',        // Biz identity, ledger green
...
export default function BizClient({ puzzles = [], questionsByNum = {}, forceNum = null }) {
  ...
  const LOFT = isLoft('biz');
  const STAGE = isStage('biz', searchParams);
  ...
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('biz');
  const STAGE_ACC = { '--stg-acc-dk': gameColor('biz'), '--stg-acc-lt': gameColorLight('biz'), '--stg-onramp-lt': gameOnrampLight('biz'), '--stg-acc-ink-lt': gameAccentInkLight('biz') };
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_biz_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_biz_day');
      }
    } catch (e) {}
  ...
  const iq = useIqStanding({ game: 'biz', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'biz', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'biz', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'biz', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'biz', active: LOFT && !playing });
  ...
  const REC_KEY = `sot_biz_rec_${PUZZLE.num}`;
  ...
  function shareUrl() { return withRef(`mindloftdaily.com/biz${isTodays ? '' : `?p=${PUZZLE.num}`}`); }
  function shareText() {
    ...
    const head = won
      ? `Biz #${PUZZLE.num} · ran the table, 25/25 · ${elapsed}${streakBit}`
      : `Biz #${PUZZLE.num} · ${depth} straight · ${elapsed}${streakBit}`;
    return `${head}\n${bar}\n${shareUrl()}`;
  }
  function copyShare() {
    const text = playing
      ? `Biz #${PUZZLE.num} — the daily business gauntlet from Mind Loft. Twenty-five questions, one life.\n${shareUrl()}`
      : shareText();
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="Twenty-five questions on business, one life."
      steps={[
        <>Answer multiple-choice business questions until you get one wrong. <b>Every question you clear is a point.</b></>,
        ...
        <>The twenty-five climb in <b>five rounds of five</b>, from gimmes to genuinely obscure, each round cycling the same five lanes: brands and products, markets and money, founders and bosses, deals and disasters, and business history.</>,
      ]}
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && (
      <DailyChrome slug="biz" name="Biz" collapsed={started} loft={LOFT} />
      )}
      ...
      {LOFT && (
        <Cap gameKey="biz" quizId={PUZZLE.quizId}
          name="Biz"
          cat="Trivia"
      ...
      <div className="sp-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        {!LOFT && (
        <DailyMasthead
          slug="biz" num={PUZZLE.num} dateLabel={PUZZLE.dateLabel} accent={COLORS.accent}
          blockGap={5} helpTop={13} marginBottom={16} onHelp={() => setShowHelp(true)}
          blocks={'BIZ'.split('').map((ch, i) => (
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Biz is ready'}</div>
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: INK, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>Twenty-five questions about business, easy to expert, {Q_SECONDS} seconds each, and one life. Answer until you miss; every question you clear is a point. The clock starts when you do.</p>
              </div>
            )}
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Biz in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new twenty-five drops at midnight Eastern.'}
                    {prevPuzzle && (<>{' '}Meanwhile: <a href={`/biz?p=${prevPuzzle.num}`} style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>run yesterday&rsquo;s questions &rarr;</a></>)}
                  </>
                ) : (
                  <>
                    You&rsquo;re playing the {PUZZLE.dateLabel.replace(', 2026', '')} archive.{' '}
                    <a href="/biz" style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Biz &rarr;</a>
            ...
            <LoftFinish
              ...
              gameRank={allTime && allTime.ready
                ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '\u2014',
                    label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Biz all time` : 'all-time rank' }
                : null}
              day={dayStats}
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Biz', sub: `No. ${prevPuzzle.num}, yesterday\u2019s puzzle`, href: `/biz?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="biz" name="Biz" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          ...
          {!LOFT && (
          <DailyGamesGrid replay={!playing ? resetGame : null} self="biz" maxWidth={620}
            challengeHref={`/duel/new?quiz=${encodeURIComponent(PUZZLE.quizId)}`}
            share={{ label: copied ? 'Copied' : 'Share', onClick: copyShare }} light
            boardSlot={<DailyBoardPanel self="biz" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Biz to your Home Screen</div>
              {isIosDevice() ? (
      ...
      {!playing && !endClosed && !LOFT && (
        <DailyEndCard modal self="biz" won={won}
      ...
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: INK }}>About Biz</h2>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Biz is a free daily business quiz from Mind Loft. Twenty-five multiple-choice questions climb from ones anyone can answer to ones almost nobody can, and a single wrong answer ends the run. Your score is simply how many you cleared in a row, which makes every question a small act of nerve: the further you get, the more you have to lose.
        </p>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Everyone plays the same twenty-five questions in the same order each day, so the daily leaderboard is a straight fight: longest run wins, and ties break by time. Twenty seconds a question keeps it honest. Every round cycles the same five lanes, brands and products, markets and money, founders and bosses, deals and disasters, and business history, so a run rewards someone who reads widely rather than someone who follows one industry. Every answer is a settled fact, never a share price or a job title that moves next week.
        </p>
