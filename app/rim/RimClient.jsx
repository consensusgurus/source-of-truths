...
// Rim — the daily outside sudoku.
//
// A 9×9 sudoku with NO digits printed inside the grid. Outside some row and
// column ends, the three digits that go in that line's first three squares,
// reading in from the edge, are printed as an unordered set. The blank
// gutters are the puzzle: the ramp is how many of the thirty-six triples are
// printed, Mon 30 down to Sat 16, and the Sunday Edition prints thirteen.
//
// The board is an 11×11 grid: the 9×9 sudoku in the middle, the gutter on all
// four sides. Select a square and every printed triple that speaks about it
// lights up, and a gutter whose three squares are all filled marks itself
// right or wrong.
//
// Nothing about the RULES changes in code. A triple constrains which digits
// go where but never lets a digit repeat, so the note scrubber, the peer
// highlight and the win check are the ordinary sudoku ones. FREE is all 81
// squares, since nothing is given.
//
...
// date on the server (app/rim/page.js), per-puzzle localStorage saves,
// /rim?p=N archive pinning, streaks, and the shared /api/quiz/* flow.
...
// Rim's legacy slate hue is MOSS. The page itself is painted by the Sudoku
// category ramp (lib/category-ramp.js), so this pair only tells Rim apart
...
// cyan, Frame amber and Whittle brown.
...
const COLORS = {
  ...
  accent: ACCENT,
  accentSoft: '#f1f8e6',
  accentTint: '#d9efb4',       // a square holding the highlighted digit
  accentPick: '#bfe38a',       // the selected square
  accentDeep: '#365314',       // pressed / shadow
  green: T.successDeep,
};
...
const HELP_KEY = 'sot_rim_help_seen';
const STATS_KEY = 'sot_rim_stats';
...
export default function RimClient({ puzzles = [], forceNum = null }) {
  const PUZZLE = useMemo(() => pickPuzzle(puzzles, forceNum), [puzzles, forceNum]);
  const GIVEN = PUZZLE.given;
  const SOL = PUZZLE.sol;
  const STORE_KEY = `sot_rim_${PUZZLE.num}`;
  // The gutter clues, by side: a three-digit string, or null where the gutter is blank.
  const GUTTER = PUZZLE.rim;
  ...
  useEffect(() => { if (stats) setHintOk(hintAllowed('rim', stats)); }, [stats]);
  useEffect(() => { if (g.hintUsed) spendHint('rim'); }, [g.hintUsed]);
  ...
  const LOFT = isLoft('rim');
  const STAGE = isStage('rim', searchParams);
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('rim');
  const Cap = STAGE ? StageChrome : LoftCap;
  const STAGE_ACC = { '--stg-acc-dk': gameColor('rim'), '--stg-acc-lt': gameColorLight('rim'), '--stg-onramp-lt': gameOnrampLight('rim'), '--stg-acc-ink-lt': gameAccentInkLight('rim') };
  ...
  const iq = useIqStanding({ game: 'rim', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'rim', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'rim', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'rim', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'rim', active: LOFT && !playing });
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_rim_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_rim_day');
      }
    } catch (e) {}
  ...
  const REC_KEY = `sot_rim_rec_${PUZZLE.num}`;
  ...
  function shareText() {
    // Nine green squares, one line. No board state leaks: it is all nine or
    // none, exactly like the score.
    const squares = won ? '\u{1F7E9}'.repeat(N) : '⬜'.repeat(N);
    const hintBit = g.hintUsed ? ' · \u{1F4A1}' : '';
    const streakBit = isTodays && myStats.cur >= 2 ? ` · streak ${myStats.cur}` : '';
    const head2 = won
      ? `Rim #${PUZZLE.num}${PUZZLE.sunday ? ' · Sunday' : ''} · solved in ${elapsed}${hintBit}${streakBit}`
      : `Rim #${PUZZLE.num} · gave up`;
    return `${head2}\n${squares}\n${shareUrl()}`;
  }
  function shareUrl() {
    return withRef(`mindloftdaily.com/rim${isTodays ? '' : `?p=${PUZZLE.num}`}`);
  }
  function copyShare() {
    const text = playing
      ? `Rim #${PUZZLE.num} — the daily outside sudoku from Mind Loft.\n${shareUrl()}`
      : shareText();
  ...
  const tripleState = (side, k) => {
    ...
    return { full: true, ok: vals.slice().sort().join('') === clue };
  };
  ...
  function gutterCell(side, k) {
    ...
    const cls = `ri-gut ri-${side}${on ? ' on' : ''}${st && st.full ? ' done' : ''}`;
    if (clue == null) return <div key={`${side}${k}`} className={cls} aria-hidden="true" />;
    const line = side === 'top' || side === 'bottom' ? `Column ${k + 1}` : `Row ${k + 1}`;
    const edge = side === 'top' ? 'top' : side === 'bottom' ? 'bottom' : side === 'left' ? 'left' : 'right';
    const title = `${line}: its three squares nearest the ${edge} edge hold ${clue.split('').join(', ')} in some order${st && st.full ? (st.ok ? '. They do.' : '. They do not yet.') : ''}`;
    return (
      <div key={`${side}${k}`} className={cls} title={title}>
        <span>{clue.split('').join(' ')}</span>
        {st && st.full && <span className={`ri-mark${st.ok ? ' ok' : ' off'}`} aria-hidden="true">{st.ok ? '\u2713' : '\u2717'}</span>}
      </div>
    );
  }
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="A sudoku with nothing printed inside the grid. Instead, outside some row and column ends, the three digits that go in the first three squares reading in from that edge are printed, in no particular order. Rows, columns and boxes still hold 1 to 9 exactly once, and the blank gutters are the puzzle."
      steps={[
      ...
      knack="A printed triple is a naked triple handed to you: those three squares hold exactly those three digits, so every other square in their box loses all three at once. Two triples that meet in a box between them name six of its nine digits, which leaves the other three squares of the box holding the remaining three. Select any square and the gutters that speak about it light up."
      footer="Every board has exactly one solution and can always be reached by logic, with no guessing. Solve it and you score a perfect 10, and because nothing is counted against you, the daily leaderboard is a straight race on the clock. One free hint, on your first ever play, fills a correct number. Printed triples fall through the week from 30 of 36 on Monday to 16 on Saturday, and the Sunday Edition prints thirteen."
    />
  );
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && <Grain />}
      {!STAGE && (
      <DailyChrome slug="rim" name="Rim" collapsed={started} loft={LOFT} />
      )}
      {/* LOFT: the cap replaces the title block AND the board's own stat strip.
          Rim is scored all or nothing, so the outcome is only ever won or
          lost; there is no partial state for the amber cap to carry. */}
      {LOFT && (
        <Cap gameKey="rim" quizId={PUZZLE.quizId}
          name="Rim"
          cat="Sudoku"
          ...
          sunday={PUZZLE.sunday ? 'Sunday Edition · Thirteen clues' : null}
          figures={[
            { v: elapsed, k: 'time' },
      ...
      <div className="ri-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style>{`
          @media(max-width:560px){.ri-wrap{padding-left:12px !important;padding-right:12px !important;}}
          .ri-btn{font-family:${SANS};font-weight:800;font-size:14px;border:2px solid ${STAGE ? 'var(--stg-line2)' : 'var(--blue-deep)'};background:${STAGE ? 'transparent' : 'var(--white)'};color:${STAGE ? 'var(--stg-ink)' : 'var(--blue-deep)'};border-radius:8px;padding:9px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:7px;}
          .ri-btn:hover{background:var(--stg-surf2, ${COLORS.accentSoft});}
          .ri-cell{display:flex;align-items:center;justify-content:center;font-family:${MONO};box-sizing:border-box;cursor:pointer;position:relative;user-select:none;-webkit-tap-highlight-color:transparent;min-width:0;min-height:0;overflow:hidden;}
          .ri-given{font-weight:700;color:${INK};}
          .ri-user{font-weight:500;color:var(--stg-acc-ink, ${COLORS.accent});}
          .ri-notes{display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);width:100%;height:100%;padding:2px;box-sizing:border-box;}
          .ri-note{display:flex;align-items:center;justify-content:center;font-family:${MONO};font-size:9px;line-height:1;color:var(--stg-ink2, #8a93a3);}
          /* the gutter: mono, tight, and leaning toward the grid it labels */
          .ri-gut{display:flex;align-items:center;justify-content:center;gap:2px;font-family:${MONO};font-weight:500;font-size:clamp(8.5px,2.3vw,12px);color:${INK};box-sizing:border-box;user-select:none;line-height:1;white-space:nowrap;}
          .ri-gut.ri-top{flex-direction:column;justify-content:flex-end;padding-bottom:3px;}
          .ri-gut.ri-bottom{flex-direction:column;justify-content:flex-start;padding-top:3px;}
          .ri-gut.ri-left{justify-content:flex-end;padding-right:4px;}
          .ri-gut.ri-right{justify-content:flex-start;padding-left:4px;}
          .ri-mark{font-family:${MONO};font-weight:700;font-size:clamp(8px,2vw,11px);line-height:1;letter-spacing:0;}
          .ri-mark.ok{color:var(--stg-good, ${COLORS.green});}
          .ri-mark.off{color:var(--stg-bad, ${COLORS.rust});}
          .ri-gut.on{color:var(--stg-acc-ink, ${COLORS.accent});font-weight:700;background:color-mix(in srgb, var(--stg-acc, ${COLORS.accent}) 16%, transparent);border-radius:4px;}
          .ri-gut.done{color:var(--stg-mute, #c3c8cf);}
          .ri-pad{width:100%;aspect-ratio:1;border-radius:9px;border: 1.5px solid var(--stg-line, rgba(28,30,36,0.5));background:${STAGE ? 'var(--stg-surf)' : 'var(--white)'};font-family:${MONO};font-weight:500;color:${INK};cursor:pointer;display:flex;align-items:center;justify-content:center;position:relative;box-shadow:0 2px 0 rgba(28,30,36,0.4);}
          .ri-pad:active{transform:translateY(1px);box-shadow:0 1px 0 rgba(28,30,36,0.4);}
          .ri-pad.done{color:var(--stg-mute2, #c3c8cf);box-shadow:none;background:${STAGE ? 'var(--stg-surf2)' : '#f4f5f7'};cursor:default;}
          .ri-pad.done span{text-decoration:line-through;}
          .ri-pad.armed{background:var(--stg-acc, ${COLORS.accent});color:var(--stg-onramp, var(--white));border-color:var(--stg-acc, ${COLORS.accent});box-shadow:0 2px 0 var(--stg-acc, ${COLORS.accentDeep});}
          .ri-pad.armed .ri-pad-n{color:${STAGE ? 'color-mix(in srgb, var(--stg-onramp, #08222e) 68%, transparent)' : COLORS.accentTint};}
          .ri-pad .ri-pad-n{position:absolute;bottom:2px;right:4px;font-size:8px;color:var(--stg-mute2, #aab0bb);font-weight:500;}
          .ri-tool{font-family:${SANS};font-weight:800;font-size:12.5px;border:1.5px solid ${STAGE ? 'var(--stg-line2)' : 'rgba(28,30,36,0.35)'};background:${STAGE ? 'var(--stg-surf2)' : 'var(--white)'};color:${INK};border-radius:8px;padding:7px 11px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;}
          .ri-tool.on{background:${STAGE ? STAGE_C : COLORS.ink};color:${STAGE ? 'var(--stg-onramp, #08222e)' : 'var(--white)'};border-color:${STAGE ? STAGE_C : COLORS.ink};}
        `}</style>
        ...
        <DailyMasthead
          ...
          marginBottom={16}
          onHelp={() => setShowHelp(true)}
          sunday={PUZZLE.sunday && <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: `var(--stg-onramp, ${T.white})`, background: `var(--stg-acc, ${COLORS.accent})`, borderRadius: 4, padding: '2px 6px' }}>Sunday Edition &middot; Thirteen clues</span>}
          blocks={'RIM'.split('').map((ch, i) => (
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Rim is ready'}</div>
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: INK, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>A sudoku with nothing printed inside the grid. Outside some row and column ends, the three digits of the first three squares from that edge are printed, in no order. Rows, columns and boxes still hold 1 to 9 exactly once.</p>
              </div>
            )}
            <div style={{ marginTop: 18, display: 'flex', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <button className="ri-btn" onClick={startGame} style={{ borderColor: STAGE ? STAGE_C : undefined, background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
              <div>
          ...
          <div style={{ maxWidth: GRID_MAX + 2 * GUT_PX, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `${GUT}fr repeat(${N}, minmax(0, 1fr)) ${GUT}fr`, gridTemplateRows: `${GUT}fr repeat(${N}, minmax(0, 1fr)) ${GUT}fr`, aspectRatio: '1' }}>
              ...
              {Array.from({ length: CELLS }).map((_, idx) => {
                ...
                const cls = `ri-cell ${given ? 'ri-given' : val ? 'ri-user' : ''}`;
                return (
                  ...
                  <div className={cls} style={cellStyle(idx)}
                    ...
                    {val ? (
                      ...
                      <div className="ri-notes" style={{ position: 'relative', zIndex: 2 }}>
                        {Array.from({ length: N }).map((__, k) => (
                          <span key={k} className="ri-note">{(notes[idx] & (1 << (k + 1))) ? k + 1 : ''}</span>
                        ))}
                      </div>
                    ) : null}
                  </div>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${N}, minmax(0, 1fr))`, gap: 5, maxWidth: GRID_MAX, margin: '16px auto 0' }}>
                {Array.from({ length: N }).map((_, k) => {
                  ...
                  return (
                    <button key={d} className={`ri-pad${done ? ' done' : ''}${armed === d ? ' armed' : ''}`} onClick={() => { if (!done) padTap(d); }} aria-label={`enter ${d}`}>
                      <span style={{ fontSize: 'clamp(15px, 4.5vw, 21px)' }}>{d}</span>
                      {!done && <span className="ri-pad-n">{N - (padCounts[d] || 0)}</span>}
                    </button>
                  );
                })}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center', marginTop: 10, flexWrap: 'wrap' }}>
                <button className={`ri-tool${noteMode ? ' on' : ''}`} onClick={() => setNoteMode((m) => !m)} title="Toggle pencil notes (N)">
                  <Pencil size={14} /> Notes {noteMode ? 'on' : 'off'}
                </button>
                <button className="ri-tool" onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)" style={{ opacity: canUndo ? 1 : 0.4, cursor: canUndo ? 'pointer' : 'default' }}>
                  <RotateCcw size={14} /> Undo
                </button>
                <button className="ri-tool" onClick={() => eraseCell(sel)} title="Erase selected square (Backspace)">
                  <Eraser size={14} /> Erase
                </button>
                <button className="ri-tool" onClick={clearBoard} disabled={!hasEntries}
                ...
                {hintOk && !g.hintUsed && (
                  <button className="ri-tool" onClick={useHint} title="Fill one correct square (one hint, first play only)" style={{ background: `var(--stg-surf, ${COLORS.accentSoft})`, borderColor: 'rgba(153,27,27,0.5)', color: ACC_DEEP_INK }}>
        ...
        {started && (
          ...
          {!playing && (
            <div style={{ maxWidth: GRID_MAX + 76, margin: '0 auto' }}>
              {PUZZLE.sunday && (
                <div style={{ fontSize: 12.5, fontWeight: 600, color: FADED, fontStyle: 'italic', margin: '10px 0 0' }}>The Sunday Edition — thirteen printed triples, against thirty on a Monday, and every other gutter is blank.</div>
              )}
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Rim in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new outside sudoku drops at midnight Eastern.'}
                    {prevPuzzle && (
                      <>
                        {' '}Meanwhile:{' '}
                        <a href={`/rim?p=${prevPuzzle.num}`} style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>
                          play yesterday&rsquo;s Rim &rarr;
                        </a>
                      </>
                    )}
                  </>
                  ...
                  <>
                    ...
                    <a href="/rim" style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Rim &rarr;</a>
                    {' · '}
          ...
          <LoftFinish
            ...
            gameRank={allTime && allTime.ready
              ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '—',
                  label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Rim all time` : 'all-time rank' }
              : null}
            day={dayStats}
            ...
            options={[
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Rim', sub: `No. ${prevPuzzle.num}, yesterday's puzzle`, href: `/rim?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="rim" name="Rim" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          {LOFT && (
            <div className={STAGE ? undefined : 'loft-report'}>
              <ReportIssue self="rim" name="Rim" accent="#ffffff" align="center" onHelp={() => setShowHelp(true)} />
            </div>
          )}
          ...
          <DailyGamesGrid replay={!playing ? resetGame : null}
            ...
            light
            boardSlot={<DailyBoardPanel self="rim" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider
          />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Rim to your Home Screen</div>
              {isIosDevice() ? (
                <ol style={{ margin: '0 0 4px', paddingLeft: 20, color: INK, fontSize: 14, lineHeight: 1.7 }}>
                  <li>Tap the <b>Share</b> button in Safari&apos;s toolbar.</li>
                  <li>Scroll down and tap <b>Add to Home Screen</b>.</li>
                  <li>Tap <b>Add</b> &mdash; the tile opens today&apos;s outside sudoku, every day.</li>
                </ol>
              ) : (
                <p style={{ margin: '0 0 4px', color: INK, fontSize: 14, lineHeight: 1.7 }}>
                  Open your browser&apos;s menu and choose <b>Add to Home Screen</b> (or <b>Install app</b>). The tile opens today&apos;s outside sudoku, every day.
                </p>
              )}
      ...
      {showHelp && (
        <div onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 460, background: STAGE ? 'var(--stg-raise,#0e131f)' : COLORS.cream, borderRadius: 12, border: STAGE ? '1px solid var(--stg-line)' : `2px solid ${COLORS.ink}`, padding: '20px 22px', fontFamily: SANS, maxHeight: '86vh', overflowY: 'auto' }}>
            ...
            {rulesBody}
            <button className="ri-btn" onClick={() => { setShowHelp(false); try { localStorage.setItem(HELP_KEY, '1'); } catch (e) {} }} style={{ marginTop: 14, background: COLORS.ink, color: T.white }}>Play</button>
          </div>
        </div>
      )}
      ...
      <StageFold />
      {/* About Rim — crawlable prose, server-rendered into the HTML */}
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: INK }}>About Rim</h2>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Rim is a free daily outside sudoku from Mind Loft. Each day gives you a 9×9 grid with no digits printed inside it at all. Instead, outside some of the row and column ends, the three digits that appear in that line's first three squares reading in from the edge are printed, in no particular order, and rows, columns and 3×3 boxes still hold 1 through 9 exactly once. The gutters that are left blank are the puzzle.
        </p>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          There is no arithmetic anywhere: a printed triple says which three digits sit in those three squares, not where, so it reads as a naked triple you did not have to find, and two triples meeting in a box name six of its digits between them. Select a square and every gutter that speaks about it lights up. Every board has a single solution you can always reach by logic, with no guessing. Wrong entries are never flagged, so spotting your own slips is part of the puzzle, and a clean solve scores a perfect 10 with the daily leaderboard decided on the clock.
        </p>
        <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          A new board drops every day at midnight Eastern; weekdays print fewer triples as the week goes on, from 30 of 36 on Monday down to 16 on Saturday, and the Sunday Edition prints thirteen. No app, no signup, play free in your browser. Want the rest of the family? Try <a href="/suds" style={{ color: INK, fontWeight: 800 }}>Suds</a>, the classic 9×9, <a href="/sando" style={{ color: INK, fontWeight: 800 }}>Sando</a>, the sandwich sudoku, <a href="/frame" style={{ color: INK, fontWeight: 800 }}>Frame</a>, the one with the sums, and <a href="/diag" style={{ color: INK, fontWeight: 800 }}>Diag</a>, the one with the diagonals.
        </p>
      </section>
