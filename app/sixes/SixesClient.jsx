...
// Sixes — the daily 6×6 mini sudoku.
//
// Each day: a 6×6 grid of the digits 1–6, in six boxes two rows tall and three
// columns wide. Fill every empty square so no digit repeats in any row, column,
// or box. There is exactly one solution, and it is always reachable by pure
// logic. Classic sudoku manners: a wrong digit is NOT flagged, it looks like any
// other entry, and the grid is accepted only once every square is right.
//
// THE LANE IS SPEED. Four sudoku variants already ship (Suds, Quilt, Cages,
// Sando) and every one of them is a 9×9 you sit down with. This is the one you
// finish standing up, so a solve is a flat 10 and the daily leaderboard is a
// straight race on the clock. That is why there is no error count anywhere in
// this file, and why `miss` is null on the registry row.
//
// Forked from app/suds/SudsClient.jsx and re-based on 6×6 geometry: 36 squares,
// digits 1–6, boxOf below, and the heavy rules falling after column 2 and rows
// 1 and 3. Same daily plumbing as every other board: banked puzzles gated by
// Eastern date on the server (app/sixes/page.js), per-puzzle localStorage saves,
// /sixes?p=N archive pinning, streaks, and the shared /api/quiz/* flow.
...
// Sixes identity is BLUE, the one hue absent from the whole Numbers row (Suds
// orange, Quilt magenta, Carve violet, Cipher and Sando teal, Tally and Pricer
// green, Crunch amber, Blitz olive, Cages purple). Every colour the board uses
...
const HELP_KEY = 'sot_sixes_help_seen';
const STATS_KEY = 'sot_sixes_stats';

// ─── 6×6 geometry. Every number below is the whole reason this is not Suds. ──
const N = 6;                 // digits, and the side of the grid
const CELLS = 36;
const BOX_H = 2;             // a box is 2 rows tall
const BOX_W = 3;             // and 3 columns wide
const boxOf = (r, c) => Math.floor(r / BOX_H) * BOX_H + Math.floor(c / BOX_W);
const GRID_MAX = 396;        // px — six 66px squares, comfortable under a thumb
...
export default function SixesClient({ puzzles = [], forceNum = null }) {
  const PUZZLE = useMemo(() => pickPuzzle(puzzles, forceNum), [puzzles, forceNum]);
  const GIVEN = PUZZLE.given;
  const SOL = PUZZLE.sol;
  const STORE_KEY = `sot_sixes_${PUZZLE.num}`;
  ...
  useEffect(() => { if (stats) setHintOk(hintAllowed('sixes', stats)); }, [stats]);
  useEffect(() => { if (g.hintUsed) spendHint('sixes'); }, [g.hintUsed]);
  ...
  const STAGE_ACC = { '--stg-acc-dk': gameColor('sixes'), '--stg-acc-lt': gameColorLight('sixes'), '--stg-onramp-lt': gameOnrampLight('sixes'), '--stg-acc-ink-lt': gameAccentInkLight('sixes') };
  ...
  const iq = useIqStanding({ game: 'sixes', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'sixes', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'sixes', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'sixes', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'sixes', active: LOFT && !playing });
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_sixes_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_sixes_day');
      }
    } catch (e) {}
  ...
  // A digit placed all six times greys out on the pad. This counts EVERY
  ...
  const REC_KEY = `sot_sixes_rec_${PUZZLE.num}`;
  ...
  // desktop keyboard: arrows move, 1–6 fill, 0/Backspace erase, N toggles notes
  const onKey = useCallback((e) => {
    ...
    if (/^[1-6]$/.test(k)) { enterDigit(sel, Number(k)); return; }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function shareText() {
    // Six squares, because the game is called Sixes. No board state leaks:
    // it is all six or none, exactly like the score.
    const squares = won ? '\u{1F7E6}'.repeat(N) : '⬜'.repeat(N);
    const hintBit = g.hintUsed ? ' · \u{1F4A1}' : '';
    const streakBit = isTodays && myStats.cur >= 2 ? ` · streak ${myStats.cur}` : '';
    const head2 = won
      ? `Sixes #${PUZZLE.num}${PUZZLE.sunday ? ' · Sunday' : ''} · solved in ${elapsed}${hintBit}${streakBit}`
      : `Sixes #${PUZZLE.num} · gave up`;
    return `${head2}\n${squares}\n${shareUrl()}`;
  }
  function shareUrl() {
    return withRef(`mindloftdaily.com/sixes${isTodays ? '' : `?p=${PUZZLE.num}`}`);
  }
  function copyShare() {
    const text = playing
      ? `Sixes #${PUZZLE.num} — the daily mini sudoku from Mind Loft.\n${shareUrl()}`
      : shareText();
  ...
  // The heavy rules fall on the BOX edges, which on a 6×6 means after column 2
  // and after rows 1 and 3. Getting this wrong is the fastest way to make a
  // mini sudoku unreadable, so it is derived from BOX_W / BOX_H, not typed out.
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="Fill every empty square so each row, each column and each box holds the digits 1 to 6 with no repeats. A box is one of the six blocks two squares tall and three wide, marked off by the heavy lines."
      steps={[
    ...
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      <div className="sx-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .sx-btn:hover{background:var(--stg-surf2, ${COLORS.accentSoft});}
          ...
          .sx-user{font-weight:500;color:var(--stg-acc-ink, ${COLORS.accent});}
        ...
        <DailyMasthead
          ...
          marginBottom={16}
          onHelp={() => setShowHelp(true)}
          sunday={PUZZLE.sunday && <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 500, color: `var(--stg-onramp, ${T.white})`, background: `var(--stg-acc, ${COLORS.accent})`, borderRadius: 4, padding: '2px 6px' }}>Sunday Edition &middot; Hard</span>}
          blocks={'SIXES'.split('').map((ch, i) => (
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginTop: 18, display: 'flex', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <button className="sx-btn" onClick={startGame} style={{ borderColor: STAGE ? STAGE_C : undefined, background: STAGE ? STAGE_C : T.cta, color: STAGE ? 'var(--stg-onramp, #08222e)' : T.white, fontSize: 15, padding: '11px 22px' }}>Start</button>
              <div>
          ...
          <div style={{ maxWidth: GRID_MAX, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${N}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${N}, minmax(0, 1fr))`, aspectRatio: '1', border: '2.5px solid var(--stg-line3, rgba(28,30,36,0.85))', borderRadius: 4, overflow: 'hidden' }}>
              {Array.from({ length: CELLS }).map((_, idx) => {
                ...
                const cls = `sx-cell ${given ? 'sx-given' : val ? 'sx-user' : ''}`;
                return (
                  <div key={idx} className={cls} style={cellStyle(idx)}
                    ...
                    {val ? (
                      <span style={{ fontSize: 'clamp(20px, 7vw, 30px)' }}>{val}</span>
                    ) : notes[idx] ? (
                      <div className="sx-notes">
                        {Array.from({ length: N }).map((__, k) => (
                          <span key={k} className="sx-note">{(notes[idx] & (1 << (k + 1))) ? k + 1 : ''}</span>
                        ))}
                      </div>
                    ) : null}
                  </div>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${N}, minmax(0, 1fr))`, gap: 6, maxWidth: GRID_MAX, margin: '16px auto 0' }}>
                {Array.from({ length: N }).map((_, k) => {
                  ...
                  return (
                    <button key={d} className={`sx-pad${done ? ' done' : ''}${armed === d ? ' armed' : ''}`} onClick={() => { if (!done) padTap(d); }} aria-label={`enter ${d}`}>
                      <span style={{ fontSize: 'clamp(18px, 5.5vw, 25px)' }}>{d}</span>
                      {!done && <span className="sx-pad-n">{N - (padCounts[d] || 0)}</span>}
                    </button>
                  );
                })}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center', marginTop: 10, flexWrap: 'wrap' }}>
                <button className={`sx-tool${noteMode ? ' on' : ''}`} onClick={() => setNoteMode((m) => !m)} title="Toggle pencil notes (N)">
                  <Pencil size={14} /> Notes {noteMode ? 'on' : 'off'}
                </button>
                <button className="sx-tool" onClick={undo} disabled={!canUndo} title="Undo (Ctrl+Z)" style={{ opacity: canUndo ? 1 : 0.4, cursor: canUndo ? 'pointer' : 'default' }}>
                  <RotateCcw size={14} /> Undo
                </button>
                <button className="sx-tool" onClick={() => eraseCell(sel)} title="Erase selected square (Backspace)">
                  <Eraser size={14} /> Erase
                </button>
                <button className="sx-tool" onClick={clearBoard} disabled={!hasEntries}
                ...
                {hintOk && !g.hintUsed && (
                  <button className="sx-tool" onClick={useHint} title="Fill one correct square (one hint, first play only)" style={{ background: `var(--stg-surf, ${COLORS.accentSoft})`, borderColor: 'rgba(29,78,216,0.5)', color: ACC_DEEP_INK }}>
                ...
                {isTodays ? (
                  <>
                    ...
                    {prevPuzzle && (
                      <>
                        {' '}Meanwhile:{' '}
                        <a href={`/sixes?p=${prevPuzzle.num}`} style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>
                          play yesterday&rsquo;s Sixes &rarr;
                        </a>
                      </>
                    )}
                  </>
                  ...
                  <>
                    ...
                    <a href="/sixes" style={{ color: `var(--stg-ink, ${COLORS.ember})`, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Sixes &rarr;</a>
                    {' · '}
        ...
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          {LOFT && (
            <div className={STAGE ? undefined : 'loft-report'}>
              <ReportIssue self="sixes" name="Sixes" accent="#ffffff" align="center" onHelp={() => setShowHelp(true)} />
            </div>
          )}
          ...
          <DailyGamesGrid replay={!playing ? resetGame : null}
            ...
            light
            boardSlot={<DailyBoardPanel self="sixes" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider
          />
          )}
