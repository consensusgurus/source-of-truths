...
// Script, the daily film and television gauntlet.
//
// Twenty-five questions on film and television stand between you and a perfect
// run. They climb from gimme to expert in five tiers of five, every tier cycling
// the same five lanes (movies, television, actors and directors, awards and box
// office, and behind the scenes), and everyone faces the same twenty-five in the
// same order. You have twenty seconds a question and one life: answer wrong, or
...
//
// EVERY FACT IN THE BANK IS FROZEN, and film and television rot in a particular
// way: a part is recast, a series is renewed or cancelled, a gross is overtaken,
// a record falls. So the bank asks only about settled things. Never who
// currently plays anything, never what is currently the highest grossing or the
// longest running, never how an unfinished show ends. A superlative worth asking
// is pinned in the stem instead ("the highest grossing film of the 1990s"), and
// casting questions are asked about performances already given.
//
// Streak asks whether you know a little about everything and Deep asks how far
// down one subject you can go. Script asks how much of the screen you have
// actually watched, and how much of it you remember.
...
const COLORS = {
  ...
  accent: '#4a1d6b',        // Script identity
  accentSoft: '#fbeee6', green: T.successDeep,
};
const SANS = "'Manrope', system-ui, -apple-system, sans-serif";
const MONO = "'DM Mono', ui-monospace, 'SFMono-Regular', monospace";
const HELP_KEY = 'sot_script_help_seen';
const STATS_KEY = 'sot_script_stats';
...
export default function ScriptClient({ puzzles = [], questionsByNum = {}, forceNum = null }) {
  const PUZZLE = useMemo(() => pickPuzzle(puzzles, forceNum), [puzzles, forceNum]);
  const QUESTIONS = questionsByNum[PUZZLE.num] || [];
  const STORE_KEY = `sot_script_${PUZZLE.num}`;
  ...
  const LOFT = isLoft('script');
  const STAGE = isStage('script', searchParams);
  ...
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('script');
  const STAGE_ACC = { '--stg-acc-dk': gameColor('script'), '--stg-acc-lt': gameColorLight('script'), '--stg-onramp-lt': gameOnrampLight('script'), '--stg-acc-ink-lt': gameAccentInkLight('script') };
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_script_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_script_day');
      }
    } catch (e) {}
  ...
  const iq = useIqStanding({ game: 'script', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'script', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'script', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'script', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'script', active: LOFT && !playing });
  ...
  const REC_KEY = `sot_script_rec_${PUZZLE.num}`;
  ...
  function shareUrl() { return withRef(`mindloftdaily.com/script${isTodays ? '' : `?p=${PUZZLE.num}`}`); }
  function shareText() {
    ...
    const head = won
      ? `Script #${PUZZLE.num} · ran the table, 25/25 · ${elapsed}${streakBit}`
      : `Script #${PUZZLE.num} · ${depth} straight · ${elapsed}${streakBit}`;
    return `${head}\n${bar}\n${shareUrl()}`;
  }
  function copyShare() {
    const text = playing
      ? `Script #${PUZZLE.num} — the daily film and television gauntlet from Mind Loft. Twenty-five questions, one life.\n${shareUrl()}`
      : shareText();
  ...
  const rulesBody = (
    <DailyRules
      ...
      lead="Twenty-five questions on film and television, one life."
      steps={[
        <>Answer multiple-choice film and television questions until you get one wrong. <b>Every question you clear is a point.</b></>,
        ...
        <>The twenty-five climb in <b>five rounds of five</b>, from gimmes to genuinely obscure, each round cycling the same five lanes: movies, television, actors and directors, awards and box office, and behind the scenes.</>,
      ]}
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && (
      <DailyChrome slug="script" name="Script" collapsed={started} loft={LOFT} />
      )}
      ...
      {LOFT && (
        <Cap gameKey="script" quizId={PUZZLE.quizId}
          name="Script"
          cat="Trivia"
      ...
      <div className="sp-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        {!LOFT && (
        <DailyMasthead
          slug="script" num={PUZZLE.num} dateLabel={PUZZLE.dateLabel} accent={COLORS.accent}
          blockGap={5} helpTop={13} marginBottom={16} onHelp={() => setShowHelp(true)}
          blocks={'SCRIPT'.split('').map((ch, i) => (
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Script is ready'}</div>
            {gateRules ? rulesBody : (
              <div style={{ fontSize: 14, lineHeight: 1.55, color: INK, fontWeight: 600 }}>
                <p style={{ margin: '0 0 6px' }}>Twenty-five questions about film and television, easy to expert, {Q_SECONDS} seconds each, and one life. Answer until you miss; every question you clear is a point. The clock starts when you do.</p>
              </div>
            )}
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Script in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new twenty-five drops at midnight Eastern.'}
                    {prevPuzzle && (<>{' '}Meanwhile: <a href={`/script?p=${prevPuzzle.num}`} style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>run yesterday&rsquo;s questions &rarr;</a></>)}
                  </>
                ) : (
                  <>
                    You&rsquo;re playing the {PUZZLE.dateLabel.replace(', 2026', '')} archive.{' '}
                    <a href="/script" style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Script &rarr;</a>
            ...
            <LoftFinish
              ...
              gameRank={allTime && allTime.ready
                ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '\u2014',
                    label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Script all time` : 'all-time rank' }
                : null}
              day={dayStats}
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Script', sub: `No. ${prevPuzzle.num}, yesterday\u2019s puzzle`, href: `/script?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="script" name="Script" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          ...
          {!LOFT && (
          <DailyGamesGrid replay={!playing ? resetGame : null} self="script" maxWidth={620}
            challengeHref={`/duel/new?quiz=${encodeURIComponent(PUZZLE.quizId)}`}
            share={{ label: copied ? 'Copied' : 'Share', onClick: copyShare }} light
            boardSlot={<DailyBoardPanel self="script" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Script to your Home Screen</div>
              {isIosDevice() ? (
      ...
      {!playing && !endClosed && !LOFT && (
        <DailyEndCard modal self="script" won={won}
      ...
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: INK }}>About Script</h2>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Script is a free daily film and television quiz from Mind Loft. Twenty-five multiple-choice questions climb from ones anyone can answer to ones almost nobody can, and a single wrong answer ends the run. Your score is simply how many you cleared in a row, which makes every question a small act of nerve: the further you get, the more you have to lose.
        </p>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Everyone plays the same twenty-five questions in the same order each day, so the daily leaderboard is a straight fight: longest run wins, and ties break by time. Twenty seconds a question keeps it honest. Every round cycles the same five lanes, movies, television, actors and directors, awards and box office, and behind the scenes, so a run rewards someone who watches widely rather than someone who follows one genre. Every answer is a settled fact, never a cast list or a record that moves next month.
        </p>
