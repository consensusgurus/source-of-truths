...
// Junkyard — the biggest lot in the family. Same jam, eight by eight.
//
// Nearly thirty blocks, and the red one has to reach the gap in the right wall.
// Every block is locked to one axis. Tap a block, tap where you want it, and it
// slides there if the lane is clear. Eight is even, so the exit lane cannot sit
// on a true middle rank: it takes row 4, four ranks of traffic above it and
// three below, the opposite lean to Parker's row 2 on a 6x6, so the two
// even-sided boards in the family do not jam the same way.
//
// EIGHT BY EIGHT IS THE CEILING, and not by choice. lib/jam-core packs
// occupancy into two 32-bit words, so 64 cells is the largest board the engine
// can hold, and it throws rather than truncate past it. There is no fourth game
// in this family without a different representation.
//
// WHY A THIRD GAME RATHER THAN A HARDER IMPOUND. The same reason Impound was
// not a wider Parker: a board size is frozen into every banked board, every
// stored perfect and every leaderboard row behind it. So the family is a
// LADDER of three fixed sizes rather than one game that grows, and a player who
// finishes any of them is handed the next one they have not played (see
// GAME_FAMILIES in lib/daily-games.js).
//
// THE LADDER'S STEP SHORTENS AT THIS RUNG, and that is measured rather than
// chosen. Impound's Monday opens exactly where Parker's hardest weekday rung
// begins; repeating that step would put Junkyard's Monday at 28 and, compounded
// through the within-week floor, its Sunday floor near 50 against a climb
// ceiling of about 54, which is the coin-toss Impound's own Sunday floor had to
// be measured down twice to avoid. So Junkyard opens at 22 to 28: clear of
// Impound's whole Monday band, deeper than any Parker board short of a Sunday,
// and reachable every week.
//
...
// app/junkyard/solver.js. A rule fixed in one game must be fixed in the engine,
...
// server (app/junkyard/page.js), per-puzzle localStorage saves, /junkyard?p=N
...
const COLORS = {
  ...
  accent: '#7c5c2e',        // Junkyard identity — weathered tarmac gold
...
const HELP_KEY = 'sot_junkyard_help_seen';
const STATS_KEY = 'sot_junkyard_stats';
...
export default function JunkyardClient({ puzzles = [], forceNum = null }) {
  const PUZZLE = useMemo(() => pickPuzzle(puzzles, forceNum), [puzzles, forceNum]);
  const STORE_KEY = `sot_junkyard_${PUZZLE.num}`;
  ...
  useEffect(() => { if (stats) setHintOk(hintAllowed('junkyard', stats)); }, [stats]);
  useEffect(() => { if (g.hintUsed) spendHint('junkyard'); }, [g.hintUsed]);
  ...
  const LOFT = isLoft('junkyard');
  const STAGE = isStage('junkyard', searchParams);
  const STAGE_C = STAGE ? 'var(--stg-acc)' : gameColor('junkyard');
  const Cap = STAGE ? StageChrome : LoftCap;
  const STAGE_ACC = { '--stg-acc-dk': gameColor('junkyard'), '--stg-acc-lt': gameColorLight('junkyard'), '--stg-onramp-lt': gameOnrampLight('junkyard'), '--stg-acc-ink-lt': gameAccentInkLight('junkyard') };
  ...
  useEffect(() => {
    ...
    try {
      if (PUZZLE.num === pickPuzzle(puzzles, null).num) {
        ...
        if (done || g.t0) localStorage.setItem('sot_junkyard_day', JSON.stringify({ d: etToday(), done }));
        else localStorage.removeItem('sot_junkyard_day');
      }
    } catch (e) {}
  ...
  const iq = useIqStanding({ game: 'junkyard', quizId: PUZZLE.quizId, active: LOFT && !playing });
  const nextUp = useNextUnplayed({ self: 'junkyard', active: LOFT && !playing });
  const upNext = useUnplayedSimilar({ self: 'junkyard', active: LOFT && !playing });
  const dailyBoard = useDailyBoard({ quizId: PUZZLE.quizId, active: LOFT && !playing });
  const allTime = useGameAllTime({ game: 'junkyard', active: LOFT && !playing });
  const dayStats = useDayStats();
  const catRank = useCategoryRank({ self: 'junkyard', active: LOFT && !playing });
  const prevPuzzle = puzzles.find((x) => x.num === PUZZLE.num - 1) || null;
  const myStats = deriveStats(stats, pickPuzzle(puzzles, null).num);

  const REC_KEY = `sot_junkyard_rec_${PUZZLE.num}`;
  ...
  function shareUrl() { return withRef(`mindloftdaily.com/junkyard${isTodays ? '' : `?p=${PUZZLE.num}`}`); }
  function shareText() {
    ...
    const head = won
      ? `Junkyard #${PUZZLE.num}${PUZZLE.sunday ? ' · Sunday' : ''} · ${used} moves, ${vs} · ${elapsed}${hintBit}${streakBit}`
      : `Junkyard #${PUZZLE.num} · left it junkyarded`;
    return `${head}\n${squares}\n${shareUrl()}`;
  }
  function copyShare() {
    const text = playing
      ? `Junkyard #${PUZZLE.num} \u2014 the bigger daily sliding-block jam from Mind Loft. An eight by eight lot, you are in the red one, and there is one gap in the wall.\n${shareUrl()}`
      : shareText();
  ...
  return (
    <div className={STAGE ? 'stage-page' : (LOFT ? 'loft-page' : undefined)}
      ...
      {!STAGE && (
      <DailyChrome slug="junkyard" name="Junkyard" collapsed={started} loft={LOFT} />
      )}
      {/* LOFT: the cap replaces the title block AND the board's own stat
          strip. Junkyard grades a win from 10 down, so any solve is a win and
          a give-up is not: there is no partial state for the amber cap. */}
      {LOFT && (
        <Cap gameKey="junkyard" quizId={PUZZLE.quizId}
          name="Junkyard"
          cat="Logic"
      ...
      <div className="pk-wrap" style={{ position: 'relative', zIndex: 2, maxWidth: 1180, margin: '0 auto', padding: '18px 38px 80px', fontFamily: SANS }}>
        ...
        {!LOFT && (
        <DailyMasthead
          slug="junkyard" num={PUZZLE.num} dateLabel={PUZZLE.dateLabel} accent={COLORS.accent}
        ...
        {preStart && (
          <div className={STAGE ? 'stg-gate' : undefined} style={{ background: STAGE ? SURF : COLORS.cream, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 12, padding: '22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 10 }}>{gateRules ? 'How to play' : 'Junkyard is ready'}</div>
            {gateRules ? rulesBody : (
        ...
        <div className={STAGE ? 'stg-board' : (LOFT ? 'loft-card' : undefined)} style={{ background: STAGE ? SURF : T.white, border: STAGE ? `1px solid ${SURF_B}` : `2px solid ${COLORS.ink}`, borderRadius: 10, padding: '13px 15px 15px', boxShadow: STAGE ? 'none' : '5px 5px 0 rgba(28,30,36,0.16)', marginBottom: 12 }}>
          ...
          <div style={{ marginTop: 12, minHeight: 22, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: SANS, fontSize: 13, fontWeight: 800, color: playing ? `var(--stg-acc-ink, ${COLORS.accent})` : `var(--stg-mute, ${COLORS.faded})` }}>
              {!playing
                ? (won ? (used === perfect ? `Out in ${used}. That is perfect.` : `Out in ${used}. Par was ${par}.`) : 'You left it junkyarded.')
          ...
          {!playing && (
            <div style={{ maxWidth: 472, margin: '0 auto' }}>
              ...
              <p className={STAGE ? undefined : 'loft-tailnote'} style={{ fontSize: 12, color: FADED, fontWeight: 600, margin: '12px 0 0' }}>
                {isTodays ? (
                  <>
                    {countdown ? <>Next Junkyard in <b style={{ color: INK, fontVariantNumeric: 'tabular-nums' }}>{countdown}</b>.</> : 'A new board drops at midnight Eastern.'}
                    {prevPuzzle && (<>{' '}Meanwhile: <a href={`/junkyard?p=${prevPuzzle.num}`} style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>play yesterday&rsquo;s Junkyard &rarr;</a></>)}
                  </>
                ) : (
                  <>
                    You&rsquo;re playing the {PUZZLE.dateLabel.replace(', 2026', '')} archive.{' '}
                    <a href="/junkyard" style={{ color: COLORS.ember, fontWeight: 800, textDecoration: 'underline' }}>Back to today&rsquo;s Junkyard &rarr;</a>
            ...
            <LoftFinish
              ...
              gameRank={allTime && allTime.ready
                ? { value: allTime.rank != null ? `#${Number(allTime.rank).toLocaleString()}` : '\u2014',
                    label: allTime.field != null ? `of ${Number(allTime.field).toLocaleString()} Junkyard all time` : 'all-time rank' }
                : null}
              day={dayStats}
              ...
              prevPuzzle && { tone: 'another', label: 'Play another Junkyard', sub: `No. ${prevPuzzle.num}, yesterday\u2019s puzzle`, href: `/junkyard?p=${prevPuzzle.num}` },
        ...
        {!STAGE && <GamePanel self="junkyard" name="Junkyard" onShow={() => setShowChrome(true)} />}
        <div style={{ display: (focusMode && !STAGE) ? 'none' : 'block', margin: '30px auto 0' }}>
          {LOFT && (
            <div className={STAGE ? undefined : 'loft-report'}>
              <ReportIssue self="junkyard" name="Junkyard" accent="#ffffff" align="center" onHelp={() => setShowHelp(true)} />
            </div>
          )}
          {!LOFT && (
          <DailyGamesGrid replay={!playing ? resetGame : null} self="junkyard" maxWidth={620}
            challengeHref={`/duel/new?quiz=${encodeURIComponent(PUZZLE.quizId)}`}
            share={{ label: copied ? 'Copied' : 'Share', onClick: copyShare }} light
            boardSlot={<DailyBoardPanel self="junkyard" quizId={PUZZLE.quizId} maxWidth={620} streak={{ current: myStats.cur, best: myStats.max }} />}
            divider />
          )}
        ...
        {showA2hsHelp && (
          <div onClick={() => setShowA2hsHelp(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(20,22,28,0.55)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: STAGE ? 'var(--stg-raise,#0e131f)' : T.white, borderRadius: 14, maxWidth: 430, width: '100%', padding: '22px 22px 16px', fontFamily: SANS, border: STAGE ? '1px solid var(--stg-line)' : '1.5px solid rgba(20,22,28,0.12)' }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: INK, marginBottom: 8 }}>Add Junkyard to your Home Screen</div>
              {isIosDevice() ? (
      ...
      {!playing && !endClosed && !LOFT && (
        <DailyEndCard modal self="junkyard" won={won}
      ...
      <section style={{ position: 'relative', display: (focusMode && !STAGE) ? 'none' : 'block', zIndex: 2, maxWidth: 620, margin: '0 auto', padding: '10px 24px 42px', fontFamily: SANS }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: INK }}>About Junkyard</h2>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Junkyard is a free daily sliding-block puzzle from Mind Loft, and it is the biggest lot in the family: eight by eight, with close to thirty blocks that each slide on one axis only, and a red block that has to reach the gap in the wall. Tap a block, tap where you want it, and it goes if the lane is clear. Parker is the same puzzle on a six by six lot and Impound on a seven by seven, so the three run as one ladder of sizes.
        </p>
        <p style={{ margin: '0 0 8px', fontSize: 13, lineHeight: 1.65, color: FADED, fontWeight: 600 }}>
          Every board is machine generated and then solved exactly, by breadth-first search over the whole reachable position, so perfect really is the fewest moves that exist rather than somebody&rsquo;s guess. Par sits a cushion above perfect: it is the number a clean solve lands on, and it is beatable. Boards climb through the week, from the low twenties on Monday to the low forties by Saturday, and the Sunday Edition runs longer again. Every rung sits above the matching rung on Impound, and Monday alone already runs deeper than any Parker board short of a Sunday.
        </p>
