...
import {
  ridgeLS, bradleyTerry, bradleyTerryFit, devigTwoWay, sd, scaleTo, rankDesc, rankDescTied, mean,
} from './gridiron-math';
...
export const PARAMS = {
  ...
  cfb: { cap: 28, lam: 1, hbt: 0.30, mix: { margin: 0, wins: 0, cover: 1 }, adj: 0.5, beta: 0.105, n0: 0, coverCap: 28, scaleResume: true, tau: 0.5, tau0: 0.05, window: [1, 0.6, 0.35], linesShare: 0.75, hf: 2.5 },
...
/*
  The build date to measure a sport against, and the one to print on its page.

  A snapshot is not always rebuilt all at once: on 2026-09-11 the NFL block was
  refreshed on a Friday while CFB and MLB kept their 09-08 data. With only the
  file-level `fetchedAt` there is no honest answer to that. Stamping the file
  today claims the untouched CFB board was rebuilt today; leaving it at 09-08
  dates the NFL's fresh 09-11 sources THREE DAYS INTO THE FUTURE, and `ageOf`
  then renders "-3 days old" on the page and hands the 30-day gate a negative
  number.

  So a sport may carry its own `builtAt`, which wins for that sport and changes
  nothing for a sport that does not set one. Set it in the same edit that
  refreshes a sport's sources, and never let a source be dated after it --
  verify-gridiron.mjs check 9 enforces exactly that.
*/
export const builtAtFor = (sport) => (DATA[sport] && DATA[sport].builtAt) || DATA.fetchedAt;
...
/*
  ...
  `excluded` is the AGE-INDEPENDENT gate, set by hand at gather time, and it
  is CLAUDE-RANKINGS.md §5 rule 2 finally expressed in code: a source publishes
  only if its own CONTENT-DERIVED freshness signal matches the week being
  built. The 30-day rule cannot see that.

  First use, 2026-09-14: BCF Toys F+ printed "2026 F+ Ratings (through Week 1)"
  in its own heading and updates midweek, so on that build it was six days old,
  inside every age limit, and still describing a board that had not seen a
  single week-2 result. Scoring it would have mixed a week-1 opinion into a
  week-2 ranking under a header claiming the models carry 36% of the board.
  (F+ itself left the snapshot on 2026-09-15, replaced by its two components;
  see the file header of lib/gridiron-data.js.)

  Second use, 2026-09-15: the NFL win-totals board, which was undated, named the
  season, and so passed §5's undated path with nothing wrong with its age. What
  was wrong is that a full week of football was played and its order did not move
  a single place, while its own played-games column still read 0 for the team
  that had won on Monday night -- the frozen preseason market §3 predicted for
  win totals, in the tier where it does the most damage. It was removed from the
  snapshot outright later the same day (owner), so nothing carries `excluded`
  right now. The field stays because the next source to fail this way will need
  it, and because a source considered and rejected still has to be able to render
  with its reason rather than vanish.

  ⚠️ The inverse case, same day, is the one worth remembering: DRatings NFL
  prints 0-0 for every team even in week 2 (its own W-L column is broken; the CFB
  board on the same site is fine) and dates itself only with a relative "Updated
  N ago" string. Both of the cheap signals say STALE and the model is live -- 30
  of 32 teams moved and the moves correlate with week 1 at 0.45 against the
  closing spread. `excluded` is for a source whose CONTENT is behind, which is
  not the same thing as a source whose page furniture is broken. Measure the
  board before reaching for this field.

  The value is the reader-facing reason and it renders on the page exactly like
  an age exclusion, struck through with `EXCLUDED` where the weight goes, so the
  column is still there and still says why it is not scored. That is the rule
  from §5: a failing source is excluded from this week's composite, never
  substituted and never silently dropped.
*/
export function freshness(src, fetchedAt, season) {
  if (src.excluded) return { ok: false, age: ageOf(src, fetchedAt), why: String(src.excluded) };
...
export function computeComposite(block, sport, opts = {}) {
  ...
  const fetchedAt = opts.fetchedAt || builtAtFor(sport);
  ...
  //
  // ⚠️ CFB SCORES THE COVER TERM AND NOTHING ELSE (owner rule, 2026-09-08).
  // mix 0 / 0 / 1, n0 0, capped per game at `coverCap`, then put on the
  // market's spread. margin and wins are still COMPUTED (the ridge supplies
  // the fitted home field) and are worth zero. NFL and MLB are unchanged.
  //
  // Why both had to GO rather than be reweighted: EACH SCALES WITH GAMES
  // PLAYED AND NEITHER CAN SEE THE OPPONENT until teams share one, which in
  // week 1 they do not.
  //   * In a one-game graph the ridge splits the capped margin symmetrically,
  //     so LSU's 51-10 over Clemson and Ohio State's 56-3 over Ball State BOTH
  //     rated exactly 8.50, as did four other teams. Clemson's whole rating
  //     came from losing that game, so beating Clemson by 41 is the same solve
  //     that makes Clemson look bad and the credit cancels itself.
  //   * The phantom game divides one game by two and two games by three, so
  //     LSU rated 8.50 on a 25.5-point per-game margin while a 2-0 USC rated
  //     12.55 on a 20.6 average. Better per game, worse rating.
  //   * Bradley-Terry gave USC 10.09 to LSU's 5.12, almost exactly 2x, which
  //     is the win COUNT and nothing else.
  // So a 2-0 USC that went 1-1 against the number (+1.4 a game) held resume 1
  // over a 1-0 LSU at +37.9. Reweighting cannot fix that: no split of two
  // terms that cannot tell Clemson from Fresno State ranks quality.
  //
  // Cover is the one measurement that prices the OPPONENT and the SITE,
  // because the closing line does, and it never imports the market's opinion
  // of the team itself. Straight per-game mean, NO n/(n+n0) shrinkage: that
  // shrinkage is itself a games-played effect (0.20 at one game against 0.33
  // at two), which is the thing being removed.
  //
  // ⚠️ ACCEPTED COST, ruled on with the boards side by side. This is a
  // WHAT-YOU-DID-AGAINST-EXPECTATION column, not an absolute-quality one.
  // Ohio State sits 60th for beating Ball State by 53, because that is what
  // was expected of it, and a 1-0 Massachusetts sits 3rd for winning outright
  // at Rutgers as a 29.5-point dog. Do not "fix" either: they are the column
  // working. The alternative measured and rejected was crediting each game
  // against the opponent's MARKET rating, which reads more like a poll and put
  // USC 34th rather than 73rd, but makes the results pillar borrow the
  // market's opinion, and the entire argument for the pillar is that it is the
  // one thing on the board that is not a forecast.
  //
  // Superseded the same day: mix 0.40 / 0.20 / 0.40, itself replacing
  // 0.45 / 0.30 / 0.25. It moved ten points from wins to cover and was not
  // enough, leaving USC resume 2 on the same volume effect.
    ...
    */
    if (P.market === 'moneyline') {
      ...
      if (sp != null && Number.isFinite(sp)) {
        ...
        // Capped like a margin, and for the same reason: beating a number by
        // 45 says "far better than the market thought" and nothing finer, and
        // uncapped one blowout would own the column. The W-L-P record below is
        // the SCOREBOARD's cover and is never capped.
        const cc = P.coverCap == null ? c : Math.max(-P.coverCap, Math.min(P.coverCap, c));
        for (const [i, sign] of [[h, 1], [a, -1]]) {
          if (i >= nTeams) continue;
          ats[i].sum += sign * cc; ats[i].n++;
  ...
  */
  ...
  const Oc = centreOn(O);
  // A cover-only resume is a per-game margin against the line, whose spread is
  // far wider than a ridge rating's, so it goes onto the market's scale the
  // same way the analytics pillar does. Without that a stated 6.7% share would
  // carry a good deal more than 6.7% of the board.
  const Rc = R
    ? (P.scaleResume
      ? (() => { const c = centreOn(R); const t = scaleTo(board.map((i) => c[i]), board.map((i) => Oc[i])); const out = new Array(n).fill(0); board.forEach((i, k) => { out[i] = t[k]; }); return out; })()
      : centreOn(R))
    : null;
  ...
  // COMPETITION ranking, not index order: the cover-only resume ties by
  // construction at the cap, and printing 1, 2, 3 over three identical numbers
  // is the fabricated order section 2d bans in the MODELS column.
  const kR = Rc ? rankDescTied(board.map((i) => Rc[i])) : null;
  const kO = rankDescTied(board.map((i) => Oc[i]));
  const kM = anyLine ? rankDescTied(board.map((i) => M[i])) : null;
  const kA = Ac ? rankDescTied(board.map((i) => Ac[i])) : null;
  const rR = kR ? kR.rank : null, rO = kO.rank, rM = kM ? kM.rank : null, rA = kA ? kA.rank : null;
  const tie = (t, k) => (t && t.tied[k] ? 'T' : '');

  // ---- columns, in display order: pillar first, then its sources ----
  const coverOnly = P.mix.margin === 0 && P.mix.wins === 0;
  const columns = [];
  columns.push({
    /*
      A COVER-ONLY RESULTS PILLAR IS NOT A RÉSUMÉ AND MUST NOT BE LABELLED ONE
      (owner, 2026-09-20).

      "Résumé" reads as "who did you beat, and where". This column measures
      close to the opposite: what a team did MINUS what the closing line
      expected of it. The two diverge hard and the word gives a reader no way to
      tell. Ole Miss went 3-0 with a win over the number 5 team on this board
      and sat 103rd in this column, because one 44.5-point non-conference spread
      is a third of a three-game sample. That is the column working as specified
      and the label lying about what it is.

      Where margin and wins DO score, as in the NFL and MLB pillars, the ridge
      solves across the whole schedule and prices the opponent, so "Résumé" is
      honest there and stays. The branch is on the MATH, not on the sport, so a
      pillar that is ever reweighted to cover-only renames itself.
    */
    id: 'results', kind: 'pillar', tier: 'results', short: coverOnly ? 'vs Expected' : 'Résumé',
    coverOnly,
    label: coverOnly
      ? 'Results rating: the average of what this team did MINUS what the closing line expected of it, per game. The line already prices the opponent and the site, so a win over a good team counts for more than a bigger win over a weak one, and playing an extra game earns nothing by itself.'
      : 'Results rating: luck-adjusted margins with strength of schedule, a win rating, and performance against the spread',
  ...
  columns.push({
    ...
    label: P.market === 'moneyline'
      ...
      : 'Record against the closing spread; hover for the luck-adjusted average cover margin, capped per game like a margin',
  ...
  const rows = board.map((i, k) => {
    ...
    if (Rc && played) { shown.results = tie(kR, k) + rR[k]; ranks.results = rR[k]; pts.results = Rc[i]; }
    ...
    if (rM && hasLine[i]) { shown.lines = tie(kM, k) + rM[k]; ranks.lines = rM[k]; pts.lines = M[i] - mean(board.map((j) => M[j])); }
    else if (rM) { shown.lines = 'no line'; }
    if (Ac && modelSeen[i]) { shown.model = tie(kA, k) + rA[k]; ranks.model = rA[k]; pts.model = Ac[i]; }
