...
export function useThemePop() {
  ...
  useEffect(() => {
    ...
    // The two home cards (new games, Groups launch) are modal, so the bubble
    // waits for them as well as for the arrival screen.
    const busy = () => introLive || !!document.querySelector('.stw, .prm-scrim, .gpp-scrim');
    const tick = () => {
      if (!alive) return;
