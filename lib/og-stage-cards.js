...
// -- MIND LOFT KIDS ----------------------------------------------------------
// The /kids hub and all six kids dailies share this card: the pitch is the
...
export function renderKidsCard() {
  return renderStageCard({
    ...
    sub: 'Six gentle puzzles a day that get kids counting, sorting, spotting patterns and reasoning things out. No sign-up, no ads, nothing counts against them.',
    url: HOST + '/kids',
...
// -- A QUIZ -----------------------------------------------------------------
// Quiz titles run long ("Name the 5 Most Populous Cities in Every State"), and
// an unbounded headline at 66px ran off the right edge. Bound the box beside
// the glyph tile and step the size down as the title grows.
function quizHead(title) {
  const n = String(title || '').length;
  return { headline: clamp(title, 58), big: n <= 22 ? 66 : n <= 34 ? 58 : 48, headMax: '700px' };
}
